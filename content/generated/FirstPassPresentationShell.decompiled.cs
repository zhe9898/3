...
public static class FirstPassPresentationShell
{
	...
	{
		ClanSnapshot clan2 = bundle.Clans.OrderByDescending((ClanSnapshot clan) => clan.Prestige).ThenBy<ClanSnapshot, string>((ClanSnapshot clan) => clan.ClanName, StringComparer.Ordinal).FirstOrDefault();
		NarrativeNotificationSnapshot narrativeNotificationSnapshot = notifications.FirstOrDefault();
		...
		string title;
		string guidance;
		bool flag = HallDocketShellAdapter.TryBuildGreatHallLead(bundle.HallDocket, out title, out guidance);
		GreatHallDashboardViewModel greatHallDashboardViewModel = new GreatHallDashboardViewModel
		{
			...
			FamilySummary = BuildGreatHallFamilySummary(clan2, bundle.PlayerCommands.Affordances),
			...
			AftermathDocketSummary = WarfareCampaignShellAdapter.BuildGreatHallAftermathDocketSummary(bundle, notifications),
			LeadNoticeTitle = (flag ? title : (narrativeNotificationSnapshot?.Title ?? "堂上暂无急报")),
			LeadNoticeGuidance = (flag ? guidance : BuildLeadNoticeGuidance(bundle, narrativeNotificationSnapshot)),
			SecondaryDockets = HallDocketShellAdapter.BuildGreatHallSecondaryDockets(bundle.HallDocket)
		};
		greatHallDashboardViewModel.GovernanceSummary = GovernanceShellAdapter.BuildGreatHallGovernanceSummary(bundle, greatHallDashboardViewModel.GovernanceSummary);
		return greatHallDashboardViewModel;
	}
	...
	private static FamilyCouncilViewModel BuildFamilyCouncil(PresentationReadModelBundle bundle)
	{
		...
		PlayerCommandAffordanceSnapshot[] familyAffordances = bundle.PlayerCommands.Affordances.Where((PlayerCommandAffordanceSnapshot command) => string.Equals(command.SurfaceKey, "Family", StringComparison.Ordinal)).OrderBy<PlayerCommandAffordanceSnapshot, string>((PlayerCommandAffordanceSnapshot command) => command.TargetLabel, StringComparer.Ordinal).ThenBy<PlayerCommandAffordanceSnapshot, string>((PlayerCommandAffordanceSnapshot command) => command.CommandName, StringComparer.Ordinal)
			.ToArray();
		CommandReceiptViewModel[] recentReceipts = (from receipt in bundle.PlayerCommands.Receipts.Where((PlayerCommandReceiptSnapshot receipt) => string.Equals(receipt.SurfaceKey, "Family", StringComparison.Ordinal)).OrderBy<PlayerCommandReceiptSnapshot, string>((PlayerCommandReceiptSnapshot receipt) => receipt.TargetLabel, StringComparer.Ordinal).ThenBy<PlayerCommandReceiptSnapshot, string>((PlayerCommandReceiptSnapshot receipt) => receipt.CommandName, StringComparer.Ordinal)
		...
		string value7 = BuildLeadFamilyLifecyclePrompt(bundle.Clans, familyAffordances);
		return new FamilyCouncilViewModel
		{
			Summary = ((bundle.Clans.Count == 0) ? "祠堂暂无线头可议。" : $"祠堂今有{value}宗争势渐炽，{value2}宗起分房之议，{value3}宗可邀族老调停；另有{value4}宗急议婚事，{value5}宗承祧未稳，{value6}宗门内举哀。{value7}"),
			CommandAffordances = familyAffordances.Select((PlayerCommandAffordanceSnapshot command) => new CommandAffordanceViewModel
			{
			...
			{
				ClanNarrativeSnapshot value8;
				ClanNarrativeSnapshot narrative2 = (narrativesByClan.TryGetValue(clan.Id.Value, out value8) ? value8 : null);
	...
	private static string BuildGreatHallFamilySummary(ClanSnapshot? clan, IReadOnlyList<PlayerCommandAffordanceSnapshot> affordances)
	{
		...
		string value = BuildClanLifecyclePrompt(clan, affordances, includeClanName: false);
		if (clan.MourningLoad <= 0)
		{
			if (clan.HeirPersonId.HasValue && clan.HeirSecurity >= 40)
			{
				...
				{
					return $"{clan.ClanName}门望{clan.Prestige}，承祧稳度{clan.HeirSecurity}，可支宗务{clan.SupportReserve}。{value}";
				}
				return $"{clan.ClanName}门望{clan.Prestige}，婚议之压{clan.MarriageAlliancePressure}，添丁之望{clan.ReproductivePressure}，可支宗务{clan.SupportReserve}。{value}";
			}
			return $"{clan.ClanName}门望{clan.Prestige}，承祧未稳，后议之压在堂，可支宗务{clan.SupportReserve}。{value}";
		}
		return $"{clan.ClanName}门望{clan.Prestige}，门内举哀未毕，丧服之重{clan.MourningLoad}，可支宗务{clan.SupportReserve}。{value}";
	}
	...
	{
		string value = (clan.HeirPersonId.HasValue ? $"承祧稳度{clan.HeirSecurity}" : $"堂上尚未定承祧，后议之压{clan.InheritancePressure}");
		string value2 = ((clan.MourningLoad > 0) ? $"门内仍系丧服之重{clan.MourningLoad}" : ((clan.InfantCount > 0) ? $"门内现有襁褓{clan.InfantCount}口，添丁之后仍待护持" : $"婚议之压{clan.MarriageAlliancePressure}，添丁之望{clan.ReproductivePressure}"));
		if (!string.IsNullOrWhiteSpace(clan.LastLifecycleOutcome))
		{
			string value3 = (string.IsNullOrWhiteSpace(clan.LastLifecycleCommandLabel) ? "门内后计" : clan.LastLifecycleCommandLabel);
			return $"{value}；{value2}。近月{value3}：{clan.LastLifecycleOutcome} {BuildClanLifecyclePrompt(clan, affordances, includeClanName: false)}".Trim();
		}
		return $"{value}；{value2}。{BuildClanLifecyclePrompt(clan, affordances, includeClanName: false)}".Trim();
	}

	private static string BuildLeadFamilyLifecyclePrompt(IReadOnlyList<ClanSnapshot> clans, IReadOnlyList<PlayerCommandAffordanceSnapshot> affordances)
	{
		PlayerCommandAffordanceSnapshot playerCommandAffordanceSnapshot = (from entry in (from clan in clans
				select new
				{
					Clan = clan,
					Affordance = SelectPrimaryLifecycleAffordance(clan, affordances)
				} into entry
				where entry.Affordance != null
				orderby GetLifecyclePriority(entry.Affordance)
				select entry).ThenBy(entry => entry.Clan.ClanName, StringComparer.Ordinal)
			select entry.Affordance).FirstOrDefault();
		if (playerCommandAffordanceSnapshot == null)
		{
			return string.Empty;
		}
		return "眼下最宜先命" + playerCommandAffordanceSnapshot.TargetLabel + playerCommandAffordanceSnapshot.Label + "。";
	}

	private static string BuildLeadNoticeGuidance(PresentationReadModelBundle bundle, NarrativeNotificationSnapshot? leadNotification)
	{
	...
	private static string BuildNotificationWhatNext(NarrativeNotificationSnapshot notification, string lifecyclePrompt)
	{
	...
	private static string BuildClanLifecyclePrompt(ClanSnapshot clan, IReadOnlyList<PlayerCommandAffordanceSnapshot> affordances, bool includeClanName)
	{
		PlayerCommandAffordanceSnapshot playerCommandAffordanceSnapshot = SelectPrimaryLifecycleAffordance(clan, affordances);
		if (playerCommandAffordanceSnapshot == null)
		{
			return string.Empty;
		}
		string text = (includeClanName ? playerCommandAffordanceSnapshot.TargetLabel : string.Empty);
		string text2 = (includeClanName ? ("眼下最宜先命" + text + playerCommandAffordanceSnapshot.Label) : ("眼下宜先" + playerCommandAffordanceSnapshot.Label));
		return text2 + "：" + playerCommandAffordanceSnapshot.AvailabilitySummary;
	}

	private static PlayerCommandAffordanceSnapshot? SelectPrimaryLifecycleAffordance(ClanSnapshot clan, IReadOnlyList<PlayerCommandAffordanceSnapshot> affordances)
	{
		return affordances.Where(delegate(PlayerCommandAffordanceSnapshot command)
		{
			int result;
			if (command.IsEnabled)
			{
				ClanId? clanId = command.ClanId;
				ClanId id = clan.Id;
				if (clanId.HasValue && clanId.GetValueOrDefault() == id && string.Equals(command.SurfaceKey, "Family", StringComparison.Ordinal))
				{
					result = (IsLifecycleFamilyCommand(command.CommandName) ? 1 : 0);
					goto IL_0057;
				}
			}
			result = 0;
		...
		}).OrderBy(GetLifecyclePriority).ThenBy<PlayerCommandAffordanceSnapshot, string>((PlayerCommandAffordanceSnapshot command) => command.CommandName, StringComparer.Ordinal)
			.FirstOrDefault();
	}
	...
	private static bool IsLifecycleFamilyCommand(string commandName)
	{
		switch (commandName)
		{
		case "ArrangeMarriage":
		case "SupportNewbornCare":
		case "DesignateHeirPolicy":
		case "SetMourningOrder":
			return true;
		default:
			return false;
		}
	...
	{
		if (!string.Equals(notification.SourceModuleKey, "FamilyCore", StringComparison.Ordinal))
		{
			return false;
		}
		return notification.Traces.Any(delegate(NotificationTraceSnapshot trace)
		{
			switch (trace.EventType)
			{
			case "MarriageAllianceArranged":
			case "BirthRegistered":
			case "DeathRegistered":
			case "HeirSecurityWeakened":
				return true;
			default:
				return false;
			}
	...
	private static int GetLifecyclePriority(PlayerCommandAffordanceSnapshot command)
	{
		...
		int result = commandName switch
		{
			"SetMourningOrder" => 0, 
			"SupportNewbornCare" => 1, 
			"DesignateHeirPolicy" => 2, 
			"ArrangeMarriage" => 3, 
			_ => 10, 
		};
		if (1 == 0)
	...
	private static string BuildGreatHallPublicLifeSummary(IReadOnlyList<SettlementPublicLifeSnapshot> publicLifeSettlements, SettlementPublicLifeSnapshot leadPublicLife)
	{
		...
		int value4 = publicLifeSettlements.Count((SettlementPublicLifeSnapshot settlement) => settlement.RoadReportLag >= 50 || settlement.CourierRisk >= 50 || Math.Abs(settlement.DocumentaryWeight - settlement.MarketRumorFlow) >= 12 || (settlement.PrefectureDispatchPressure >= 60 && settlement.PublicLegitimacy < 55));
	...
	private static IReadOnlyList<CommandAffordanceViewModel> BuildSettlementPublicLifeAffordances(PresentationReadModelBundle bundle, SettlementId settlementId)
	{
		return (from command in bundle.PlayerCommands.Affordances
			where string.Equals(command.SurfaceKey, "PublicLife", StringComparison.Ordinal) && command.SettlementId == settlementId
	...
	private static IReadOnlyList<CommandReceiptViewModel> BuildSettlementPublicLifeReceipts(PresentationReadModelBundle bundle, SettlementId settlementId)
	{
		return (from receipt in bundle.PlayerCommands.Receipts
			where string.Equals(receipt.SurfaceKey, "PublicLife", StringComparison.Ordinal) && receipt.SettlementId == settlementId
	...
	{
		SettlementSnapshot[] array = bundle.Settlements.OrderBy<SettlementSnapshot, string>((SettlementSnapshot settlement) => settlement.Name, StringComparer.Ordinal).ToArray();
		for (int i = 0; i < array.Length && i < deskSandbox.Settlements.Count; i++)
		{
			SettlementSnapshot settlement2 = array[i];
			SettlementNodeViewModel settlementNodeViewModel = deskSandbox.Settlements[i];
			settlementNodeViewModel.PublicLifeCommandAffordances = BuildSettlementPublicLifeAffordances(bundle, settlement2.Id);
			settlementNodeViewModel.PublicLifeRecentReceipts = BuildSettlementPublicLifeReceipts(bundle, settlement2.Id);
			SettlementPublicLifeSnapshot settlementPublicLifeSnapshot = bundle.PublicLifeSettlements.FirstOrDefault((SettlementPublicLifeSnapshot snapshot) => snapshot.SettlementId == settlement2.Id);
			if (settlementPublicLifeSnapshot != null && !string.IsNullOrWhiteSpace(settlementPublicLifeSnapshot.ChannelSummary))
			{
				settlementNodeViewModel.PublicLifeSummary += settlementPublicLifeSnapshot.ChannelSummary;
			}
		}
	}
	...
	private static DeskSandboxViewModel BuildDeskSandbox(PresentationReadModelBundle bundle, IReadOnlyList<NarrativeNotificationSnapshot> notifications)
	{
		...
		Dictionary<int, SettlementGovernanceLaneSnapshot> governanceBySettlement = bundle.GovernanceSettlements.ToDictionary((SettlementGovernanceLaneSnapshot settlement) => settlement.SettlementId.Value, (SettlementGovernanceLaneSnapshot settlement) => settlement);
		...
		return new DeskSandboxViewModel
		{
			Settlements = bundle.Settlements.OrderBy<SettlementSnapshot, string>((SettlementSnapshot settlement) => settlement.Name, StringComparer.Ordinal).Select(delegate(SettlementSnapshot settlement)
			{
				...
				SettlementGovernanceLaneSnapshot value5;
				bool flag4 = governanceBySettlement.TryGetValue(settlement.Id.Value, out value5);
				CampaignFrontSnapshot value6;
				bool flag5 = campaignsBySettlement.TryGetValue(settlement.Id.Value, out value6);
				CampaignMobilizationSignalSnapshot value7;
				bool flag6 = mobilizationSignalsBySettlement.TryGetValue(settlement.Id.Value, out value7);
				...
				HallDocketShellAdapter.SettlementHallAgendaProjection settlementHallAgendaProjection = HallDocketShellAdapter.BuildSettlementHallAgenda(bundle.HallDocket, settlement.Id);
				SettlementNodeViewModel settlementNodeViewModel = new SettlementNodeViewModel
				{
					...
					CampaignSummary = WarfareCampaignShellAdapter.BuildSettlementCampaignSummary(flag5 ? value6 : null, flag6 ? value7 : null, settlement, tradeRoutes),
					AftermathSummary = WarfareCampaignShellAdapter.BuildSettlementAftermathSummary(settlement, populationSettlementSnapshot, flag3 ? value4 : null, flag5 ? value6 : null, notifications),
					...
					HallAgendaSummary = settlementHallAgendaProjection.Summary,
					HallAgendaItems = settlementHallAgendaProjection.Items,
					HallAgendaCount = settlementHallAgendaProjection.Count,
					HallAgendaLaneKeys = settlementHallAgendaProjection.LaneKeys,
					HasLeadHallAgendaItem = settlementHallAgendaProjection.HasLeadItem,
					LeadHallAgendaLaneKey = settlementHallAgendaProjection.LeadLaneKey
				};
				settlementNodeViewModel.GovernanceSummary = GovernanceShellAdapter.BuildSettlementGovernanceSummary(settlementNodeViewModel.GovernanceSummary, flag4 ? value5 : null);
				return settlementNodeViewModel;
			}).ToArray()
		};
	}
	...
	private static string RenderOfficeTitle(string officeTitle)
	{
		...
		string text = officeTitle switch
		{
	...
	private static string BuildOfficePressureSummary(bool hasAppointment, string lastOutcome, int petitionBacklog, string promotionLabel, string demotionLabel)
	{
		...
		string text = lastOutcome switch
		{
	...
	private static string DescribeMobilizationWindowChinese(string mobilizationWindowLabel)
	{
		...
		string text = mobilizationWindowLabel switch
		{
			"Open" => "可开", 
			"Narrow" => "可守", 
			"Preparing" => "待整", 
...
Latest version is '10.0.0.8330' (yours is '9.0.0.7889')
