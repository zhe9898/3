...
- current public-life/order closure is committed through v19: `OrderAndBanditry`, `OfficeAndCareer`, `FamilyCore`, `PopulationAndHouseholds`, and `SocialMemoryAndRelations` each keep their own command/state/residue authority while `PlayerCommandService` and `PlayerCommandCatalog` route and describe commands
...
- current v19 adds projection-only follow-up affordance hints for repeat/switch/cooldown readback; code, docs, architecture tests, integration tests, and Unity/presentation tests are part of the current branch evidence
...
- For public-life/order work, preserve the current v19 split: each owning module resolves its own command/traces, SocialMemory owns durable social residue, PopulationAndHouseholds owns home-household local response state, and Application/Unity copy projections only. v19 follow-up hints remain projection-only and do not become a cooldown ledger or rule layer.
