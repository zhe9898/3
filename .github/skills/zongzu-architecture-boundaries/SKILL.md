...
- current mainline is implemented through v476: v109-v204 closes the first Chain 8 court-policy rule-density branch; v213-v292 implements and closes the first social mobility / fidelity-ring substrate and influence readback; v253-v268 closes Chain 9 regime-legitimacy first readback; v293-v380 closes personnel-flow command-readiness, owner-lane gate, desk echo, containment, and future Family/Office/Warfare preflight visibility without adding direct movement lanes; v381-v452 closes commoner/social-position readback, owner-source keys, scale-budget readback, regional guard, commoner status owner-lane preflight, and fidelity scale-budget preflight; v453-v460 adds household mobility dynamics explanation as runtime projection over existing household signals; v461-v468 closes that layer as governance evidence only; v469-v476 adds household mobility owner-lane preflight for future rule depth
...
- v35-v476 use structured metadata/query snapshots, existing owner state, projection-only readbacks, or docs/tests preflight evidence only: canal-window Trade/Order, household-family sponsor pressure, Family relief, Office policy implementation/readback, Office/Family lane closure, Force/Campaign owner-lane readback, Warfare directive/aftermath readback, Court-policy readback/guards, social mobility fidelity-ring/pool readbacks, regime-legitimacy readback, personnel-flow readiness/gate/future-lane preflight, commoner/social-position readback/source keys/scale-budget summaries, household mobility dynamics explanation/closeout/preflight, and closeout/preflight audits add no schema, migration, ledger, household target field, social-class engine, `PersonRegistry` domain expansion, or UI/Unity authority unless a future rule-density pass explicitly plans it
...
- fidelity promotion, commoner status, and social-position depth need a target scope, owner module, touched-count budget, deterministic order/cap, cadence, projection field, save/schema decision, and no-touch proof before becoming runtime rules; projection-only readbacks must not imply hidden full-population simulation.
- topology or feature-pack growth must declare node/edge owners, command/query/event seams, event metadata, fanout caps, deterministic traversal order, no-touch boundary, dormant/future-contract status, projection surface, manifest/save membership, startup/load cost, and rollback path; Zongzu supports additive manifest-gated feature packs, not unsafe runtime plugin marketplaces.
- pluggability means registered feature-pack manifests and content/rule extension points; it does not mean arbitrary runtime assemblies, reflection-heavy startup, UI-loaded rules, or save-shape mutation outside owner modules.
...
- For public-life/order/court-policy/regime/social-mobility/personnel-flow/social-position/household-mobility work, preserve the current v476 split: each owning module resolves its own command/traces, SocialMemory owns durable social residue on its later cadence, PopulationAndHouseholds owns household livelihood/activity/pools and household mobility dynamics plus the default first future owner lane for household mobility depth, PersonRegistry owns identity plus existing `FidelityRing` only, Office/PublicLife own court-policy/regime local readback, and Application/Unity copy projections only. v19-v476 readback/preflight fields, including `SocialPositionReadbackSummary`, `SocialPositionSourceModuleKeys`, `SocialPositionScaleBudgetReadbackSummary`, and `MobilityDynamicsExplanationSummary`, do not become cooldown ledgers, owner-lane ledgers, follow-up/docket/suggested-action/receipt/receipt-docket/public-life-receipt/mobility/personnel/future-lane/status/route-history ledgers, command resolvers, ranking rules, movement commands, or rule layers.
