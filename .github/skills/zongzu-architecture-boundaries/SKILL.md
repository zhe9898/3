...
- current public-life/order closure is committed through v24: `OrderAndBanditry`, `OfficeAndCareer`, `FamilyCore`, `PopulationAndHouseholds`, and `SocialMemoryAndRelations` each keep their own command/state/residue authority while `PlayerCommandService` and `PlayerCommandCatalog` route and describe commands
- current save anchors include `OrderAndBanditry` schema `9`, `OfficeAndCareer` schema `7`, `FamilyCore` schema `8`, `PopulationAndHouseholds` schema `3`, and `SocialMemoryAndRelations` schema `3`
...
- current v19 adds projection-only follow-up affordance hints for repeat/switch/cooldown readback; v20-v24 add projection-only owner-lane return guidance, `承接入口`, `归口状态`, and `归口后读法` readback over existing structured snapshots
...
- For public-life/order work, preserve the current v24 split: each owning module resolves its own command/traces, SocialMemory owns durable social residue, PopulationAndHouseholds owns home-household local response state, and Application/Unity copy projections only. v19 follow-up hints and v20-v24 owner-lane return/status/outcome readback remain projection-only and do not become a cooldown ledger, owner-lane ledger, command resolver, or rule layer.
