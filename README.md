...
```powershell
.\tools\Verify-CleanSolution.ps1
```

The script runs `build` and `test --no-build` serially and clears lingering repo-scoped `testhost` processes first, which keeps local validation output free of intermittent file-lock noise.

If you need the raw commands, they remain:
