...
## Household mobility runtime pool tie-break no-touch v621-v628 integration note
...
## Household mobility runtime threshold extraction v677-v684 integration note
...
## Household mobility runtime score-weight extraction v693-v700 integration note
...
## Household mobility runtime grain-floor extraction v709-v716 integration note
...
## Household mobility runtime grain-divisor extraction v725-v732 integration note
...
## Household mobility runtime land-trigger extraction v781-v788 integration note

V781-V788 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, or scheduler path. It extracts the land-holding trigger floor into owner-consumed `PopulationHouseholdMobilityRulesData` while preserving default floor 15.

The integration contract remains owner-consumed rules-data only. Application/UI/Unity must not calculate land eligibility, validation fallback, fanout eligibility, target ordering, or household movement outcomes.

## Household mobility runtime livelihood-trigger extraction v789-v796 integration note
...
## PopulationAndHouseholds event dispatch file split v901-v908 integration note

V901-V908 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior. It moves private event-dispatch/application helpers into a dedicated partial file inside the same owner module.
...
## PopulationAndHouseholds warfare aftermath file split v925-v932 integration note

V925-V932 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior. It moves private warfare aftermath handling into a dedicated partial file inside the same owner module.
...
## PopulationAndHouseholds grain price pressure clamp extraction v957-v964 integration note

V957-V964 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts `ComputePricePressure` final clamp bounds into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads price-pressure clamp bounds through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer grain price pressure or household pressure outcomes.

## PopulationAndHouseholds grain price level band extraction v965-v972 integration note

V965-V972 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts `ComputePricePressure` price-level threshold/score bands into validated owner-consumed rules-data.
...
## PopulationAndHouseholds grain price market tightness band extraction v981-v988 integration note

V981-V988 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts `ComputePricePressure` market-tightness threshold/score bands into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads market-tightness bands through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer market tightness or household pressure outcomes.

## PopulationAndHouseholds subsistence market dependency extraction v989-v996 integration note

V989-V996 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts `ComputeMarketDependencyPressure` livelihood score weights into validated owner-consumed rules-data.
...
## PopulationAndHouseholds subsistence labor clamp extraction v1013-v1020 integration note

V1013-V1020 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the final `ComputeSubsistenceLaborPressure` clamp into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads clamp floor/ceiling through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer subsistence labor pressure or household pressure outcomes.

## PopulationAndHouseholds subsistence grain buffer extraction v1021-v1028 integration note
...
## PopulationAndHouseholds subsistence fragility debt extraction v1037-v1044 integration note
...
## PopulationAndHouseholds subsistence fragility clamp extraction v1053-v1060 integration note

V1053-V1060 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the `ComputeSubsistenceFragilityPressure` final clamp into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads fragility clamp floor/ceiling values through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer fragility clamp pressure or household pressure outcomes.

## PopulationAndHouseholds subsistence interaction grain shortage extraction v1061-v1068 integration note

V1061-V1068 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the `ComputeSubsistenceInteractionPressure` grain-shortage window into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads interaction grain-shortage floor/ceiling values through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer interaction grain-shortage pressure or household pressure outcomes.

## PopulationAndHouseholds subsistence interaction cash-need extraction v1069-v1076 integration note

V1069-V1076 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the `ComputeSubsistenceInteractionPressure` cash-need boost into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads the interaction cash-need boost score through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer interaction cash-need pressure or household pressure outcomes.

## PopulationAndHouseholds subsistence interaction debt-boost extraction v1077-v1084 integration note
...
## PopulationAndHouseholds subsistence interaction clamp extraction v1093-v1100 integration note

V1093-V1100 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the `ComputeSubsistenceInteractionPressure` final clamp floor/ceiling into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads the interaction clamp floor/ceiling through `PopulationHouseholdMobilityRulesData` fallback getters and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer interaction clamp pressure or household pressure outcomes.

## PopulationAndHouseholds subsistence event threshold extraction v1101-v1108 integration note

V1101-V1108 adds no new cross-module command, event consumer, projection parser, event router, rules-data loader, rules-data file, scheduler path, or runtime behavior under default rules-data. It extracts the `ApplyGrainPriceSubsistencePressure` event distress threshold into validated owner-consumed rules-data.

The existing grain-price event path remains the integration seam: it reads the event threshold through `PopulationHouseholdMobilityRulesData` fallback getter and mutates only `PopulationAndHouseholds` owner state. No downstream layer parses `DomainEvent.Summary`, receipt text, projection prose, public-life lines, or docs text to infer event eligibility or household pressure outcomes.

## PopulationAndHouseholds subsistence distress delta clamp extraction v1109-v1116 integration note
...
## PopulationAndHouseholds tax season debt spike threshold extraction v1125-v1132 integration note
...
## PopulationAndHouseholds official supply debt delta clamp extraction v1141-v1148 integration note
...
## PopulationAndHouseholds official supply migration delta clamp extraction v1157-v1164 integration note
...
### PopulationAndHouseholds official supply interaction pressure extraction v1229-v1236 integration note

Official-supply interaction pressure ownership is internal to `PopulationAndHouseholds`. Upstream `OfficeAndCareer` still emits structured official-supply requisition metadata; `PopulationAndHouseholds` applies owner-owned livelihood conditions, thresholds, boost scores, resilience relief score, fallback scores, and clamp while computing the household burden profile.
...
### PopulationAndHouseholds official supply debt delta formula extraction v1245-v1252 integration note

Official-supply debt delta formula ownership is internal to `PopulationAndHouseholds`. Upstream `OfficeAndCareer` still emits structured official-supply requisition metadata; `PopulationAndHouseholds` applies owner-owned formula divisors, interaction floor, and component weights while computing the household burden profile.
...
### PopulationAndHouseholds official supply migration delta formula extraction v1261-v1268 integration note

Official-supply migration delta formula ownership is internal to `PopulationAndHouseholds`. Upstream `OfficeAndCareer` still emits structured official-supply requisition metadata; `PopulationAndHouseholds` applies owner-owned formula divisors and fragility threshold/boost while computing the household burden profile.
...
### PopulationAndHouseholds tax season interaction pressure extraction v1301-v1308 integration note

Tax-season interaction ownership is internal to `PopulationAndHouseholds`. Upstream `WorldSettlements` still emits structured tax-season events; `PopulationAndHouseholds` applies owner-owned tenant, land/labor, cash-need, resilience, fallback, and clamp rules while computing the household tax burden profile.
