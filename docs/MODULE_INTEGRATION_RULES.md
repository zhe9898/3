...
When application code needs full affordance/receipt snapshots, prefer a projection-local helper over open-coded `ModuleKey` / `SurfaceKey` / default-label initializers so metadata stays anchored to the shared catalog while heuristics remain projection-owned.
When multiple application projections need the same visible affordance or receipt ordering, share that ordering through a projection-local selector helper instead of re-copying command-priority switches into hall dockets, preview scenarios, or tests. Shared selectors keep visible ordering aligned while still remaining downstream of module authority.
When multiple application projections need the same settlement-scoped lookup pattern, prefer a projection-local read-only selector or one-pass settlement index over re-copying `SettlementId` trace filters and ad hoc `ToDictionary(...)` seams. Generic helpers may normalize traversal, but module filters and priority rules must stay at the call site. Notification scope helpers on `NotificationTraceSnapshot` / `NarrativeNotificationSnapshot` may answer source-module and settlement-scope membership only; they must not own visibility, sorting, UI policy, or authority rules.
When application and presentation both need the same pure snapshot-side ordering rule, prefer a contracts-level read-only selector over maintaining one copy in `Application` and another in shell adapters. Shared contracts selectors may rank existing snapshots, but they must not absorb command resolution, text-authoring, or module-owned consequence logic.
When multiple shell adapters or projection contexts only need to traverse command snapshots by surface and optional settlement scope, prefer a contracts-level read-only enumerator on the snapshot itself over repeated `SurfaceKey` filters. Traversal helpers may scope/filter, but caller-side ordering and presentation choice must remain local.
