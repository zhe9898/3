...
- `PresentationReadModelBuilder.PlayerCommands.OwnerLaneReturnSurface` is a projection helper over existing structured snapshots. It may show owner-lane return guidance, `承接入口`, `归口状态`, and `归口后读法`; it must not parse summaries, own command outcomes, create a cooldown ledger, or create an owner-lane ledger.
...
| `zongzu-pressure-chain` | Cross-module pressure propagation, especially Renzong thin/full chains and public-life/order/social-memory closure. | `RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md`, module event metadata, scheduler drain, off-scope tests, public-life order v3-v24 ExecPlans. | Traceable cause-effect design, minimal focused tests, explicit fanout/allocation proof. | Call a chain complete from docs alone, parse summaries as authority, fan out global pressure without locus/allocation, or describe refusal/response residue as an event-pool design. | Integration tests proving propagation, no-touch entities, metadata, projection receipts, SocialMemory-owned residue when in scope, and bounded chain complexity. |
