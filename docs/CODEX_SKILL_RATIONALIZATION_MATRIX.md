...
| --- | --- | --- | --- | --- | --- |
| `zongzu-architecture-boundaries` | Module ownership, scheduler flow, command/query/event seams, save/version contracts, extension boundaries, hot-path ownership. | `ModuleRunner<TState>`, `MonthlyScheduler`, `GameSimulation`, module `PublishedEvents` / `ConsumedEvents`, boundary docs, projection contexts, v32-v35 event-contract and canal-window owner-lane evidence. | .NET modular testability, explicit dependency discipline, measured hot-path optimization, low-allocation diagnostics. | Add a giant world manager, Application second rule layer, runtime plugin marketplace, foreign state mutation, or global cache that changes determinism. | Architecture/integration tests, event-contract tests, v35 Trade/Order canal-window consumer tests, `git diff --check`, focused compile/test lane, counter/profiler evidence when performance behavior changes. |
