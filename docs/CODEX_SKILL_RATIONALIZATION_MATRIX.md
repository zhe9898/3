...
| `zongzu-pressure-chain` | Cross-module pressure propagation, especially Renzong thin/full chains. | `RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md`, module event metadata, scheduler drain, off-scope tests. | Traceable cause-effect design and minimal focused tests. | Call a chain complete from docs alone, parse summaries as authority, fan out global pressure without locus/allocation. | Integration tests proving propagation, no-touch entities, metadata, and projection receipts. |
...
| `zongzu-save-and-schema` | Persisted module state, root/module schema versions, migrations, manifests, feature-pack save membership. | `SCHEMA_NAMESPACE_RULES.md`, `DATA_SCHEMA.md`, module `ModuleSchemaVersion`, save/load tests. | Versioned data and migration discipline. | Bump schema casually, store UI-only facts as authority, mutate foreign state during migration. | Persistence round-trip, migration tests, manifest tests, determinism checks if backfill changes behavior. |
