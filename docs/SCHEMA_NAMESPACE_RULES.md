...
- `OfficeAndCareer` uses namespace `OfficeAndCareer` with schema version `7` for the active governance-lite slice plus chain-4 amnesty de-duplication state, chain-7 clerk-capture edge state, chain-9 official-defection risk state, office-owned public-life refusal response trace state, v37 policy implementation drag over existing office fields, and v46-v52 Office-lane closure readback over existing office response / yamen fields
