...
- `SocialMemoryAndRelations` uses namespace `SocialMemoryAndRelations` with schema version `3` for clan emotional climate, person pressure-tempering ledgers, public-life order accepted/refused/partial/response residue, later response-residue decay/hardening, v8 actor-countermove back-pressure inputs, v13 home-household local response residue, and v14 home-household repeat-friction inputs stored in existing memory/narrative/climate records. v15 common-household response texture and v16 home-household response capacity read existing `PopulationAndHouseholds` fields/projections and add no SocialMemory state.
