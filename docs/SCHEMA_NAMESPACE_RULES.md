...
- `OfficeAndCareer` uses namespace `OfficeAndCareer` with schema version `7` for the active governance-lite slice plus chain-4 amnesty de-duplication state, chain-7 clerk-capture edge state, chain-9 official-defection risk state, office-owned public-life refusal response trace state, and v37 policy implementation drag over existing office fields
