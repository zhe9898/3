# PopulationAndHouseholds Subsistence Fragility Distress Extraction V1029-V1036

## Purpose

V1029-V1036 extracts the distress threshold-to-score portion of `ComputeSubsistenceFragilityPressure` into owner-consumed `PopulationHouseholdMobilityRulesData`.

This is a behavior-equivalent hardcoded-rule extraction. Default rules-data preserves the previous distress scoring: `>=80` gives `3`, `>=65` gives `2`, `>=50` gives `1`, and fallback gives `0`.
...
The owning module reads the extracted distress values through validated fallback getters when computing the private subsistence fragility pressure profile. Debt pressure, migration flag/risk contribution, and the final fragility clamp remain unchanged in this pass.
...
No persisted field, module schema version bump, root save version change, migration step, save-manifest membership change, route-history state, movement ledger, selector watermark, target-cardinality state, owner-lane ledger, cooldown ledger, fragility-distress state, pressure-profile ledger, ordering ledger, validation ledger, diagnostic state, performance cache, rules-data file, loader, content/config namespace, or serialized projection cache is added.

`PopulationAndHouseholds` remains schema `3`.

## Determinism risk

Runtime determinism risk is unchanged under default rules-data because the extracted distress bands preserve the old switch behavior and fallback score. Validation is deterministic; malformed fragility-distress config falls back through owner getters to the same defaults.
...
1. Add V1029-V1036 subsistence fragility distress extraction ExecPlan.
2. Add default threshold score bands, validation, and fallback getters to `PopulationHouseholdMobilityRulesData`.
3. Replace the `ComputeSubsistenceFragilityPressure` distress switch with an owner-consumed deterministic lookup.
...
- `dotnet test tests\Zongzu.Modules.PopulationAndHouseholds.Tests\Zongzu.Modules.PopulationAndHouseholds.Tests.csproj --no-build --filter "Name=GrainPriceSpike_DefaultFragilityDistressRulesDataMatchesPreviousBaseline|Name=GrainPriceSpike_CustomFragilityDistressRulesDataIsOwnerConsumed|Name=GrainPriceSpike_InvalidFragilityDistressRulesDataFallsBackToPreviousBaseline"`
- `dotnet test tests\Zongzu.Architecture.Tests\Zongzu.Architecture.Tests.csproj --no-build --filter "Name=Population_households_subsistence_fragility_distress_extraction_v1029_v1036_must_remain_owner_consumed_and_schema_neutral"`
...
Revert the code/docs/tests commit. No save migration, content migration, rules-data file rollback, fragility-distress state rollback, or production data rollback is required.

## Evidence log

- `dotnet build Zongzu.sln --no-restore` passed: 0 warnings, 0 errors.
- Focused `Zongzu.Modules.PopulationAndHouseholds.Tests` fragility-distress extraction tests passed: 3/3.
- Focused architecture guard `Population_households_subsistence_fragility_distress_extraction_v1029_v1036_must_remain_owner_consumed_and_schema_neutral` passed: 1/1.
- `dotnet test tests\Zongzu.Modules.PopulationAndHouseholds.Tests\Zongzu.Modules.PopulationAndHouseholds.Tests.csproj --no-build` passed: 106/106.
- `dotnet test tests\Zongzu.Architecture.Tests\Zongzu.Architecture.Tests.csproj --no-build` passed: 151/151.
