# PopulationAndHouseholds Subsistence Labor Clamp Extraction V1013-V1020

## Purpose

V1013-V1020 extracts the final subsistence labor pressure clamp from `ComputeSubsistenceLaborPressure` into owner-consumed `PopulationHouseholdMobilityRulesData`.

This is a behavior-equivalent hardcoded-rule extraction. Default rules-data preserves the previous final clamp: floor `-2`, ceiling `4`.
...
The owning module reads the extracted clamp through validated fallback getters after combining labor-capacity and dependent-count pressure. Labor-capacity bands and dependent-count bands remain the inputs; this pass only removes the literal final `-2..4` clamp.
...
Runtime determinism risk is unchanged under default rules-data because the extracted clamp preserves the old `-2..4` bounds. Validation is deterministic; malformed clamp config falls back through owner getters to the same defaults.
...
1. Add V1013-V1020 subsistence labor clamp extraction ExecPlan.
2. Add default clamp floor/ceiling, validation, and fallback getters to `PopulationHouseholdMobilityRulesData`.
3. Replace the `ComputeSubsistenceLaborPressure` literal final clamp with owner-consumed deterministic getters.
...
- `dotnet test tests\Zongzu.Modules.PopulationAndHouseholds.Tests\Zongzu.Modules.PopulationAndHouseholds.Tests.csproj --no-build --filter "Name=GrainPriceSpike_DefaultLaborClampRulesDataMatchesPreviousBaseline|Name=GrainPriceSpike_CustomLaborClampRulesDataIsOwnerConsumed|Name=GrainPriceSpike_InvalidLaborClampRulesDataFallsBackToPreviousBaseline"`
- `dotnet test tests\Zongzu.Architecture.Tests\Zongzu.Architecture.Tests.csproj --no-build --filter "Name=Population_households_subsistence_labor_clamp_extraction_v1013_v1020_must_remain_owner_consumed_and_schema_neutral"`
...
- Focused `Zongzu.Modules.PopulationAndHouseholds.Tests` labor-clamp extraction tests passed: 3/3.
- Focused architecture guard `Population_households_subsistence_labor_clamp_extraction_v1013_v1020_must_remain_owner_consumed_and_schema_neutral` passed: 1/1.
- `dotnet test tests\Zongzu.Modules.PopulationAndHouseholds.Tests\Zongzu.Modules.PopulationAndHouseholds.Tests.csproj --no-build` passed: 100/100.
- `dotnet test tests\Zongzu.Architecture.Tests\Zongzu.Architecture.Tests.csproj --no-build` passed: 149/149.
