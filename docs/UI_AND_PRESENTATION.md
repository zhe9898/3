...
- v373-v380 personnel flow future lane closeout adds no new shell surface. It closes the future-lane preflight/display layer as guidance only; shell authors must not present it as a direct personnel command board, office-service picker, kin-transfer surface, campaign-manpower UI, or migration economy.
- v381-v388 commoner social position preflight adds no new shell surface. Existing person dossier social-position labels and mobility summaries may remain visible as projected context, but UI/Unity must not present them as a class ladder, promote/demote button, zhuhu/kehu conversion control, office-service picker, trade-attachment command, or global per-person career board.
- v389-v396 commoner social position readback adds `SocialPositionReadbackSummary` to projected person dossiers. Lineage/person-inspector surfaces may display it as an owner-lane explanation only; UI/Unity must not infer class movement, route eligibility, promotion/demotion, zhuhu/kehu conversion, office service, trade attachment, or durable residue from it.
- v397-v404 social position owner-lane keys adds `SocialPositionSourceModuleKeys` to projected person dossiers. Lineage/person-inspector surfaces may use the copied key list for provenance or read-order hints, but UI/Unity must not infer source lanes from prose or turn the list into class movement, promotion/demotion, zhuhu/kehu conversion, office service, trade attachment, route eligibility, or durable residue.
- v405-v412 social position readback closeout adds no new shell field. It closes the current social-position readback/source-key layer as guidance only; shell authors must not present it as a class ladder, promote/demote board, zhuhu/kehu conversion control, office-service picker, trade-attachment command, or global per-person career browser.
- v413-v420 social position scale budget adds `SocialPositionScaleBudgetReadbackSummary` to projected person dossiers. Lineage/person-inspector surfaces may show it as a scale-budget explanation only; UI/Unity must not compute fidelity changes, select people, parse social-position prose, or turn it into a class ladder, global person browser, or movement/control surface.
- v421-v428 social position regional scale guard adds no new shell field. It protects the meaning of the existing scale-budget readback: regional/distant dossiers remain summary-first, and UI/Unity must not turn `regional summary` into a hidden picker, global person browser, class ladder, or precision-raising control.
- v429-v436 social position scale closeout adds no new shell field. It closes the current social-position visibility layer as guidance only; shell authors must not present it as a complete class system, global person browser, promote/demote board, zhuhu/kehu conversion control, office-service picker, trade-attachment command, or regional person selector.
- v437-v444 commoner status owner-lane preflight adds no new shell field. Future commoner status controls must arrive as projected owner-module affordances after `PopulationAndHouseholds` or another explicitly planned owner lane exists; UI/Unity must not create class buttons, person browsers, or status movement from current dossier readbacks.
- v445-v452 fidelity scale budget preflight adds no new shell field. UI/Unity may keep displaying projected near-detail/far-summary readbacks, but must not infer hidden people, raise precision, scan regional society, or turn far summaries into a class/status/person browser.
- v453-v460 household mobility dynamics explanation adds one projected desk readback field. Desk Sandbox may display `HouseholdMobilityDynamicsSummary` copied from the existing household pressure read model, but UI/Unity must not calculate household movement, class/status drift, fidelity changes, hidden targets, or route eligibility from that text.
- v461-v468 household mobility dynamics closeout adds no new shell field. It closes the display as first-layer explanation only; UI/Unity must not turn it into a migration command, household relocation control, class/status browser, route-history panel, hidden selector, or per-person world simulation.
- v469-v476 household mobility owner-lane preflight adds no new shell field. Future UI for household mobility must wait for projected owner-module affordances/readbacks after a real owner-lane rule exists; UI/Unity must not create move buttons, route-history panels, hidden household/person selectors, status/class browsers, or far-region person scans from current explanation text.
- v485-v492 household mobility preflight closeout adds no new shell field. It closes the owner-lane preflight as guidance only; UI/Unity must not present it as a movement surface, route-history panel, migration economy, status/class browser, selector, or hidden detail-promotion control.
- v501-v508 household mobility runtime rules-data readiness adds no new shell field. It documents future owner-rule scope and extraction candidates only; UI/Unity must not calculate household movement, choose targets, read rules-data for authority, parse projected prose, display hidden movement controls, or turn distant summaries into selectable households before `PopulationAndHouseholds` exposes projected owner results.
- v509-v516 household mobility rules-data contract preflight adds no new shell field. UI/Unity must not read future rules-data, validation errors, default fallback, modifier bands, fanout caps, or tie-break priorities to calculate movement, choose targets, raise detail, or display movement controls; they may only copy projected owner results after `PopulationAndHouseholds` exposes them.
- v517-v524 household mobility default rules-data skeleton adds no new shell field. UI/Unity must not read a future skeleton, fallback policy, parameter group, validation result, fanout cap, or tie-break priority to calculate movement, choose targets, raise detail, or display movement controls; they may only copy projected owner results after `PopulationAndHouseholds` exposes them.
- v525-v532 population households first hardcoded rule extraction adds no new shell field. UI/Unity must not read `PopulationHouseholdMobilityRulesData`, the focused member promotion cap, validation errors, or fallback policy to calculate movement, choose targets, raise detail, or display movement controls; they may only copy projected owner results after `PopulationAndHouseholds` exposes them.
- v533-v540 population households first mobility runtime rule adds no new shell field. UI/Unity must not read monthly runtime thresholds, caps, deltas, `MigrationPools`, rule validation errors, or fallback policy to calculate movement, choose targets, raise detail, or display movement controls; they may only copy projected owner fields such as existing migration pressure after `PopulationAndHouseholds` exposes them.
- v541-v548 household mobility first runtime rule closeout adds no new shell field. UI/Unity must not turn the closeout into movement controls, route-history display, target eligibility, cap calculation, validation display, or household outcome calculation; future shell readback needs a separate owner-projected field plan.
- v549-v556 household mobility runtime rule health evidence adds no new shell field. UI/Unity must not turn diagnostics/readiness evidence into movement controls, route-history display, target eligibility, cap calculation, health classification, validation display, saturation tuning, performance status, or household outcome calculation; future shell readback still needs a separate owner-projected field plan.
- v557-v564 household mobility runtime widening gate adds no new shell field. UI/Unity must not turn preflight wording into movement controls, route-history display, target eligibility, touched-count calculation, cap widening, recovery/decay tuning, health classification, performance status, or household outcome calculation; future shell readback still needs a separate owner-projected field plan.
- v565-v572 household mobility runtime touch-count proof adds no new shell field. UI/Unity must not turn focused test evidence into movement controls, route-history display, target eligibility, touched-count calculation, cap widening, recovery/decay tuning, health classification, performance status, or household outcome calculation; future shell readback still needs a separate owner-projected field plan.
- v573-v580 household mobility rules-data fallback matrix adds no new shell field. UI/Unity must not turn fallback evidence into config editing, validation authority, movement controls, route-history display, target eligibility, touched-count calculation, cap widening, recovery/decay tuning, health classification, performance status, or household outcome calculation; future shell readback still needs a separate owner-projected field plan.
- v581-v588 household mobility runtime threshold no-touch adds no new shell field, adapter field, ViewModel authority, Unity calculation, or presentation-side eligibility rule; UI may only display projected fields already produced by owner modules.
- v589-v596 household mobility runtime zero-cap no-touch adds no new shell field, adapter field, ViewModel authority, Unity calculation, or presentation-side cap/fanout rule; UI may only display projected fields already produced by owner modules.
- v597-v604 household mobility runtime zero-risk-delta no-touch adds no new shell field, adapter field, ViewModel authority, Unity calculation, or presentation-side risk-delta rule; UI may only display projected fields already produced by owner modules.
- v605-v612 household mobility runtime candidate-filter no-touch adds no new shell field, adapter field, ViewModel authority, Unity calculation, or presentation-side candidate rule; UI may only display projected fields already produced by owner modules.
