...
## v69-v76 global skeleton delta - 2026-04-26

- Force/Campaign/Regime owner-lane readback now follows the same thin closure discipline as Office and Family: Application assembles projected fields from structured `CampaignMobilizationSignalSnapshot`, `CampaignFrontSnapshot`, `JurisdictionAuthoritySnapshot`, `ClanSnapshot`, and `SocialMemoryEntrySnapshot` data after the owning modules have resolved.
- The player-facing wording intentionally says the military after-account returns to the warfare/force/regime lanes: `军务承接入口`, `Force承接读回`, `战后后账读回`, `军务后手收口读回`, `军务余味续接读回`, `军务闭环防回压`, `不是普通家户硬扛`, and `不是把军务后账误读成县门/Order后账`.
- This is not a new command system, event pool, event-chain body, campaign AI, force economy, Office override, Order repair path, or UI rule layer. `DomainEvent` remains a fact propagation tool only.
- Schema/migration impact is explicitly zero: no persisted state, module envelope, root/module schema version, migration, force/campaign closure ledger, owner-lane ledger, cooldown ledger, household target field, manager/controller layer, `PersonRegistry` expansion, or save-manifest update.
...
## v541-v548 household mobility first runtime rule closeout audit - 2026-04-30

- v541-v548 closes the V501-V540 track after the first small monthly owner rule landed.
- Design alignment: the first runtime rule remains a pressure nudge and does not become relocation, route history, status drift, or a migration economy.
...
## v629-v636 household mobility runtime score-ordering no-touch audit - 2026-05-02

The v629-v636 pass adds candidate score-ordering no-touch evidence for the first `PopulationAndHouseholds` household mobility runtime rule. The owner test proves a higher-score household outranks a lower household id under cap one, while the lower household id receives no household mobility pressure diff.

Alignment result: no runtime behavior change, no movement authority, no rules-data loader, no score formula retune, no candidate ordering retune, no route-history state, no schema drift, no `PersonRegistry` expansion, and no Application/UI/Unity calculation path.

## v637-v644 household mobility runtime pool-priority no-touch audit - 2026-05-02
...
## v717-v724 household mobility runtime land-floor extraction audit - 2026-05-03

The v717-v724 pass extracts the first `PopulationAndHouseholds` runtime rule's land-holding pressure floor into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded land-floor extraction, not a land economy retune, land pressure divisor extraction, class/status engine, or score formula expansion. Default land-holding pressure floor 20 preserves prior selected-household ordering under default rules-data, while malformed floor values fall back deterministically.

Code alignment: `PopulationAndHouseholds` remains the sole consumer; Application, presentation, Unity, persistence, and `PersonRegistry` do not read the land floor or calculate household mobility outcomes.

Validation alignment: focused owner tests cover default equivalence and malformed land-floor fallback, while an architecture guard proves no schema drift, loader/file, plugin marketplace, movement command, route-history state, filter retune, threshold retune, fanout widening, or second runtime rule.

## v725-v732 household mobility runtime grain-divisor extraction audit - 2026-05-03

The v725-v732 pass extracts the first `PopulationAndHouseholds` runtime rule's grain-store pressure divisor into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded grain-divisor extraction, not a grain economy retune, grain floor retune, land pressure divisor extraction, or score formula expansion. Default grain-store pressure divisor 2 preserves prior selected-household ordering under default rules-data, while malformed divisor values fall back deterministically.
...
## v765-v772 household mobility runtime labor-trigger extraction audit - 2026-05-03

The v765-v772 pass extracts the first `PopulationAndHouseholds` runtime rule's labor-capacity trigger ceiling into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded labor-trigger extraction, not a labor model retune, debt/distress/grain/land/livelihood trigger extraction, or score formula expansion. Default labor-capacity trigger ceiling 45 preserves prior no-touch behavior under default rules-data, while malformed threshold values fall back deterministically.
...
## v781-v788 household mobility runtime land-trigger extraction audit - 2026-05-03

The v781-v788 pass extracts the first `PopulationAndHouseholds` runtime rule's land-holding trigger floor into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded land-trigger extraction, not a land economy retune, grain/labor/debt/distress/livelihood trigger extraction, or score formula expansion. Default land-holding trigger floor 15 preserves prior no-touch behavior under default rules-data, while malformed threshold values fall back deterministically.
...
## v797-v804 household mobility runtime livelihood-score extraction audit - 2026-05-03

The v797-v804 pass extracts the first `PopulationAndHouseholds` runtime rule's livelihood score weights into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded livelihood-score extraction, not a livelihood engine retune, class/status engine, migration economy, or broader score formula expansion. Default weights `SeasonalMigrant=18`, `HiredLabor=10`, and `Tenant=6` preserve prior candidate ordering under default rules-data, while malformed weight values fall back deterministically.
...
## v845-v852 household mobility runtime pressure contribution floor extraction audit - 2026-05-03

The v845-v852 pass extracts the first `PopulationAndHouseholds` runtime rule's labor/grain/land pressure contribution floor into `PopulationHouseholdMobilityRulesData`.

Design alignment: this is hardcoded contribution-floor extraction, not a pressure floor retune, divisor retune, score formula retune, movement command, route-history model, fanout widening, or class/status selector. Default floor `0` preserves prior non-negative contribution behavior under default rules-data, while malformed floor values fall back deterministically.
...
## v901-v908 population households event dispatch file split audit - 2026-05-03

The v901-v908 pass moves private event-dispatch and event-application helpers from `PopulationAndHouseholdsModule.cs` into `PopulationAndHouseholdsModule.EventDispatch.cs`.
...
## v917-v924 population households monthly pulse file split audit - 2026-05-03

The v917-v924 pass moves private xun/month pulse helpers from `PopulationAndHouseholdsModule.cs` into `PopulationAndHouseholdsModule.MonthlyPulse.cs`.
...
## v957-v964 population households grain price pressure clamp extraction audit - 2026-05-03

The v957-v964 pass moves grain-price price-pressure clamp bounds from `ComputePricePressure` literals into validated `PopulationHouseholdMobilityRulesData` defaults and fallback getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns grain-price pressure and household subsistence outcomes; Application/UI/Unity do not calculate grain price pressure or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, class/status engine, or `PersonRegistry` expansion is added.

## v965-v972 population households grain price level band extraction audit - 2026-05-03

The v965-v972 pass moves grain-price level threshold/score bands from `ComputePricePressure` literals into validated `PopulationHouseholdMobilityRulesData` defaults and fallback getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns grain-price pressure and household subsistence outcomes; Application/UI/Unity do not calculate grain price level bands or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, class/status engine, or `PersonRegistry` expansion is added.
...
## v981-v988 population households grain price market tightness band extraction audit - 2026-05-03

The v981-v988 pass moves grain-price market tightness threshold/score bands from `ComputePricePressure` literals into validated `PopulationHouseholdMobilityRulesData` defaults and fallback getters.
...
## v1013-v1020 population households subsistence labor clamp extraction audit - 2026-05-03

The v1013-v1020 pass moves the final `ComputeSubsistenceLaborPressure` clamp from C# literals into validated `PopulationHouseholdMobilityRulesData` default floor/ceiling values and fallback getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns household subsistence outcomes; labor-capacity and dependent-count scoring remain unchanged; Application/UI/Unity do not calculate subsistence labor pressure or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, class/status engine, or `PersonRegistry` expansion is added.

## v1021-v1028 population households subsistence grain buffer extraction audit - 2026-05-03
...
## v1045-v1052 population households subsistence fragility migration extraction audit - 2026-05-03
...
## v1061-v1068 population households subsistence interaction grain shortage extraction audit - 2026-05-03

The v1061-v1068 pass moves the `ComputeSubsistenceInteractionPressure` grain-shortage window from C# literals into validated `PopulationHouseholdMobilityRulesData` default floor/ceiling getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns household subsistence outcomes; cash-need boost, debt threshold boost, resilience relief, and interaction clamp remain unchanged; Application/UI/Unity do not calculate interaction grain-shortage pressure or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, migration economy, class/status engine, or `PersonRegistry` expansion is added.

## v1069-v1076 population households subsistence interaction cash-need extraction audit - 2026-05-03
...
## v1085-v1092 population households subsistence interaction resilience relief extraction audit - 2026-05-03
...
## v1101-v1108 population households subsistence event threshold extraction audit - 2026-05-03
...
## v1133-v1140 population households official supply distress delta clamp extraction audit - 2026-05-03

The v1133-v1140 pass moves the `OfficialSupplyBurdenProfile.DistressDelta` clamp from C# literals `0..24` into validated `PopulationHouseholdMobilityRulesData` default getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns household official-supply burden outcomes and event emission; debt/labor/migration deltas and event threshold remain unchanged; Application/UI/Unity do not calculate official-supply distress delta or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, migration economy, class/status engine, or `PersonRegistry` expansion is added.

## v1141-v1148 population households official supply debt delta clamp extraction audit - 2026-05-04

The v1141-v1148 pass moves the `OfficialSupplyBurdenProfile.DebtDelta` clamp from C# literals `0..18` into validated `PopulationHouseholdMobilityRulesData` default getters.

Alignment check: this is a default-equivalent hardcoded-rule extraction. `PopulationAndHouseholds` still owns household official-supply burden outcomes and event emission; distress/labor/migration deltas and event threshold remain unchanged; Application/UI/Unity do not calculate official-supply debt delta or household pressure; no schema, migration, rules-data loader, plugin surface, route history, movement command, migration economy, class/status engine, or `PersonRegistry` expansion is added.

## v1149-v1156 population households official supply labor drop clamp extraction audit - 2026-05-04
...
### v1165-v1172 population households official supply burden event threshold extraction audit
...
### v1261-v1268 population households official supply migration delta formula extraction audit

`PopulationAndHouseholds` now owns the official-supply migration delta distress/debt divisors and fragility threshold/boost through validated rules-data. Defaults preserve the existing `OfficialSupplyBurdenProfile.MigrationDelta` formula while moving another hardcoded formula slice out of C# literals.
...
### v1221-v1228 population households official supply fragility pressure extraction audit

`PopulationAndHouseholds` now owns the official-supply fragility distress bands, debt bands, migration-risk threshold, shelter-drag threshold, fallback values, and clamp through validated rules-data. Defaults preserve the existing household fragility pressure profile while moving another hardcoded helper out of `ComputeOfficialSupplyFragilityPressure`.
...
`PopulationAndHouseholds` now owns the official-supply labor capacity bands, dependent count bands, dependent/labor ratio bonus, fallback values, and clamp through validated rules-data. Defaults preserve the existing household labor pressure profile while moving another hardcoded helper out of `ComputeOfficialSupplyLaborPressure`.
...
### v1197-v1204 population households official supply resource buffer extraction audit
...
### v1245-v1252 population households official supply debt delta formula extraction audit
...
### v1285-v1292 population households tax season labor pressure extraction audit

`PopulationAndHouseholds` now owns the tax-season labor capacity bands, dependent count bands, dependent/labor ratio bonus, fallback scores, and clamp through validated rules-data. Defaults preserve the existing `ComputeTaxLaborPressure` result while moving another hardcoded tax-season helper out of C# literals.

The pass does not add movement authority, route history, migration economy, class/status behavior, `PersonRegistry` fields, save schema, loader/plugin execution, prose parsing, or Application/UI/Unity calculation.

### v1293-v1300 population households tax season fragility pressure extraction audit
...
### v1309-v1316 population households tax season debt delta formula extraction audit

`PopulationAndHouseholds` now owns the tax-season debt-delta base score and pressure weights through validated rules-data. Defaults preserve the existing `TaxSeasonBurdenProfile.DebtDelta` result while moving another hardcoded tax-season formula out of C# literals.
