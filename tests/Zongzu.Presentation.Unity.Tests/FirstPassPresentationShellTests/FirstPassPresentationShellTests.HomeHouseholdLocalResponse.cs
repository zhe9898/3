...
public sealed partial class FirstPassPresentationShellTests
{
    ...
    public void Compose_ProjectsHomeHouseholdLocalResponsePublicLifeFieldsWithoutShellAuthority()
    {
        ...
        Assert.That(affordance.LeverageSummary, Does.Contain("外部后账归位"));
        Assert.That(affordance.LeverageSummary, Does.Contain("该走巡丁/路匪 lane"));
        Assert.That(affordance.LeverageSummary, Does.Contain("该走县门/文移 lane"));
        Assert.That(affordance.LeverageSummary, Does.Contain("该走族老/担保 lane"));
        Assert.That(affordance.LeverageSummary, Does.Contain("本户不能代修"));
