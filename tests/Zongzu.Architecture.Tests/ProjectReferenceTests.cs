...
public class ProjectReferenceTests
{
    ...
    [Test]
    public void Household_mobility_runtime_tie_break_priority_extraction_v829_v836_must_remain_owner_consumed_rules_data_without_schema_or_authority_drift()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_household-mobility-runtime-tie-break-priority-extraction-v829-v836.md"));
        string householdModuleTests = File.ReadAllText(Path.Combine(
            RepoRoot,
            "tests",
            "Zongzu.Modules.PopulationAndHouseholds.Tests",
        ...
        Assert.That(topologyIndex, Does.Contain("V829-V836 Household Mobility Runtime Tie-Break Priority Extraction"));
        Assert.That(socialStrata, Does.Contain("Current household mobility runtime tie-break priority extraction: v829-v836"));
        Assert.That(designAudit, Does.Contain("v829-v836 household mobility runtime tie-break priority extraction audit"));
        Assert.That(moduleBoundaries, Does.Contain("Household mobility runtime tie-break priority extraction v829-v836 boundary note"));
        Assert.That(integrationRules, Does.Contain("Household mobility runtime tie-break priority extraction v829-v836 integration note"));
        Assert.That(simulation, Does.Contain("Current household mobility runtime tie-break priority extraction v829-v836 note"));
        Assert.That(uiPresentation, Does.Contain("v829-v836 household mobility runtime tie-break priority extraction"));
        Assert.That(acceptance, Does.Contain("Household mobility runtime tie-break priority extraction v829-v836 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V829-V836 Household Mobility Runtime Tie-Break Priority Extraction"));
        Assert.That(skillMatrix, Does.Contain("Household Mobility Runtime Tie-Break Priority Extraction Through V836"));
        Assert.That(schemaRules, Does.Contain("household mobility runtime tie-break priority extraction v829-v836 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current household mobility runtime tie-break priority extraction v829-v836 note"));

        foreach (string requiredPlanText in new[]
                 {
                     "Target schema/migration impact: none",
                     "owner-consumed rules-data extraction",
                     "default pool tie-break remains settlement id ascending",
                     "default household tie-break remains household id ascending",
                     "Runtime behavior under default rules-data: unchanged",
                     "does not add new priority choices",
                     "No ordering retune",
                     "No score formula retune",
                     "No fanout widening",
                     "No second household mobility runtime rule",
        ...
        Assert.That(rulesData, Does.Contain("MonthlyRuntimePoolTieBreakPriority"));
        Assert.That(rulesData, Does.Contain("MonthlyRuntimeHouseholdTieBreakPriority"));
        Assert.That(rulesData, Does.Contain("DefaultMonthlyRuntimePoolTieBreakPriority"));
        Assert.That(rulesData, Does.Contain("DefaultMonthlyRuntimeHouseholdTieBreakPriority"));
        Assert.That(rulesData, Does.Contain("SettlementIdAscending"));
        Assert.That(rulesData, Does.Contain("HouseholdIdAscending"));
        Assert.That(rulesData, Does.Contain("GetMonthlyRuntimePoolTieBreakPriorityOrDefault"));
        Assert.That(rulesData, Does.Contain("GetMonthlyRuntimeHouseholdTieBreakPriorityOrDefault"));
        Assert.That(rulesData, Does.Contain("monthly_runtime_pool_tie_break_priority"));
        Assert.That(rulesData, Does.Contain("monthly_runtime_household_tie_break_priority"));
        Assert.That(rulesData, Does.Contain("Enum.IsDefined(MonthlyRuntimePoolTieBreakPriority)"));
        Assert.That(rulesData, Does.Contain("Enum.IsDefined(MonthlyRuntimeHouseholdTieBreakPriority)"));

        Assert.That(populationModule, Does.Contain("GetMonthlyRuntimePoolTieBreakPriorityOrDefault"));
        Assert.That(populationModule, Does.Contain("GetMonthlyRuntimeHouseholdTieBreakPriorityOrDefault"));
        Assert.That(populationModule, Does.Contain("ApplyMonthlyRuntimePoolTieBreak"));
        Assert.That(populationModule, Does.Contain("ApplyMonthlyRuntimeHouseholdTieBreak"));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        ...
        Assert.That(runtimeEnd, Is.GreaterThan(runtimeStart));
        string runtimeRule = populationModule[runtimeStart..runtimeEnd];
        Assert.That(runtimeRule, Does.Contain("ApplyMonthlyRuntimePoolTieBreak"));
        Assert.That(runtimeRule, Does.Contain("ApplyMonthlyRuntimeHouseholdTieBreak"));
        Assert.That(runtimeRule, Does.Contain("poolTieBreakPriority"));
        Assert.That(runtimeRule, Does.Contain("householdTieBreakPriority"));
        Assert.That(
            Regex.Matches(populationModule, @"\bApplyMonthlyHouseholdMobilityRuntimeRule\s*\(").Count,
            Is.EqualTo(2),
            "The tie-break priority extraction must not add or duplicate the monthly household mobility runtime rule path.");

        Assert.That(
            householdModuleTests,
            Does.Contain("RunMonth_FirstMobilityRuntimeRuleDefaultTieBreakPrioritiesPreservePreviousOrdering"));
        Assert.That(
            householdModuleTests,
            Does.Contain("PopulationHouseholdMobilityRulesData_InvalidMonthlyRuntimeTieBreakPrioritiesFallBackToDefault"));
        Assert.That(populationState, Does.Not.Contain("MonthlyRuntimePoolTieBreakPriority"));
        Assert.That(populationState, Does.Not.Contain("MonthlyRuntimeHouseholdTieBreakPriority"));
        Assert.That(populationState, Does.Not.Contain("HouseholdMobility"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Cooldown"));
        Assert.That(populationState, Does.Not.Contain("TieBreakLedger"));
        ...
        foreach (string authorityToken in new[]
                 {
                     ...
                     "HouseholdMobilityTieBreakInterpreter",
                     "HouseholdMobilityEventRouter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
    ...
    [Test]
    public void Household_mobility_runtime_unmatched_livelihood_score_extraction_v837_v844_must_remain_owner_consumed_rules_data_without_schema_or_authority_drift()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_household-mobility-runtime-unmatched-livelihood-score-extraction-v837-v844.md"));
        string householdModuleTests = File.ReadAllText(Path.Combine(
            RepoRoot,
            "tests",
            "Zongzu.Modules.PopulationAndHouseholds.Tests",
        ...
        Assert.That(topologyIndex, Does.Contain("V837-V844 Household Mobility Runtime Unmatched Livelihood Score Extraction"));
        Assert.That(socialStrata, Does.Contain("Current household mobility runtime unmatched livelihood score extraction: v837-v844"));
        Assert.That(designAudit, Does.Contain("v837-v844 household mobility runtime unmatched livelihood score extraction audit"));
        Assert.That(moduleBoundaries, Does.Contain("Household mobility runtime unmatched livelihood score extraction v837-v844 boundary note"));
        Assert.That(integrationRules, Does.Contain("Household mobility runtime unmatched livelihood score extraction v837-v844 integration note"));
        Assert.That(simulation, Does.Contain("Current household mobility runtime unmatched livelihood score extraction v837-v844 note"));
        Assert.That(uiPresentation, Does.Contain("v837-v844 household mobility runtime unmatched livelihood score extraction"));
        Assert.That(acceptance, Does.Contain("Household mobility runtime unmatched livelihood score extraction v837-v844 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V837-V844 Household Mobility Runtime Unmatched Livelihood Score Extraction"));
        Assert.That(skillMatrix, Does.Contain("Household Mobility Runtime Unmatched Livelihood Score Extraction Through V844"));
        Assert.That(schemaRules, Does.Contain("household mobility runtime unmatched livelihood score extraction v837-v844 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current household mobility runtime unmatched livelihood score extraction v837-v844 note"));

        foreach (string requiredPlanText in new[]
                 {
                     "Target schema/migration impact: none",
                     "owner-consumed hardcoded fallback extraction",
                     "default unmatched livelihood score remains `0`",
                     "Runtime behavior under default rules-data: unchanged",
        ...
        Assert.That(rulesData, Does.Contain("MonthlyRuntimeUnmatchedLivelihoodScoreWeight"));
        Assert.That(rulesData, Does.Contain("DefaultMonthlyRuntimeUnmatchedLivelihoodScoreWeight = 0"));
        Assert.That(rulesData, Does.Contain("GetMonthlyRuntimeUnmatchedLivelihoodScoreWeightOrDefault"));
        Assert.That(rulesData, Does.Contain("monthly_runtime_unmatched_livelihood_score_weight"));
        Assert.That(rulesData, Does.Contain("MaxMonthlyRuntimeLivelihoodScoreWeight"));

        Assert.That(populationModule, Does.Contain("GetMonthlyRuntimeUnmatchedLivelihoodScoreWeightOrDefault"));
        Assert.That(populationModule, Does.Contain("unmatchedLivelihoodScoreWeight"));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        ...
        Assert.That(runtimeEnd, Is.GreaterThan(runtimeStart));
        string runtimeRule = populationModule[runtimeStart..runtimeEnd];
        Assert.That(runtimeRule, Does.Contain("GetMonthlyRuntimeUnmatchedLivelihoodScoreWeightOrDefault"));
        Assert.That(runtimeRule, Does.Contain("unmatchedLivelihoodScoreWeight"));
        Assert.That(
            Regex.Matches(populationModule, @"\bApplyMonthlyHouseholdMobilityRuntimeRule\s*\(").Count,
            Is.EqualTo(2),
            "The unmatched-livelihood fallback extraction must not add or duplicate the monthly household mobility runtime rule path.");

        int resolverStart = populationModule.IndexOf(
            "private static int ResolveMonthlyHouseholdMobilityLivelihoodScoreWeight",
            StringComparison.Ordinal);
        int resolverEnd = populationModule.IndexOf(
            "private static string ResolveFocusPromotionReason",
            resolverStart,
            StringComparison.Ordinal);
        Assert.That(resolverStart, Is.GreaterThanOrEqualTo(0));
        Assert.That(resolverEnd, Is.GreaterThan(resolverStart));
        string scoreResolver = populationModule[resolverStart..resolverEnd];
        Assert.That(scoreResolver, Does.Contain("return unmatchedLivelihoodScoreWeight;"));
        Assert.That(scoreResolver, Does.Not.Contain("return 0;"));

        Assert.That(
            householdModuleTests,
            Does.Contain("RunMonth_FirstMobilityRuntimeRuleDefaultUnmatchedLivelihoodScorePreservesPreviousFallback"));
        Assert.That(
            householdModuleTests,
            Does.Contain("PopulationHouseholdMobilityRulesData_InvalidMonthlyRuntimeUnmatchedLivelihoodScoreWeightFallsBackToDefault"));
        Assert.That(populationState, Does.Not.Contain("MonthlyRuntimeUnmatchedLivelihoodScoreWeight"));
        Assert.That(populationState, Does.Not.Contain("HouseholdMobility"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Cooldown"));
        Assert.That(populationState, Does.Not.Contain("UnmatchedLivelihoodLedger"));
        Assert.That(populationState, Does.Not.Contain("OrderingLedger"));
        ...
        foreach (string authorityToken in new[]
                 {
                     ...
                     "HouseholdMobilityUnmatchedLivelihoodInterpreter",
                     "HouseholdMobilityEventRouter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string personRegistryToken in new[]
                 {
                     ...
                     "CommonerStatus",
                     "SocialClass",
                     "UnmatchedLivelihoodScoreExtraction",
                 })
        {
            Assert.That(personRegistrySource, Does.Not.Contain(personRegistryToken), personRegistryToken);
        }
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "HouseholdMobilityUnmatchedLivelihoodLedger",
                     "HouseholdMobilityOrderingLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
    ...
    [Test]
    public void Household_mobility_runtime_pressure_contribution_floor_extraction_v845_v852_must_remain_owner_consumed_rules_data_without_schema_or_authority_drift()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_household-mobility-runtime-pressure-contribution-floor-extraction-v845-v852.md"));
        string householdModuleTests = File.ReadAllText(Path.Combine(
            RepoRoot,
            "tests",
            "Zongzu.Modules.PopulationAndHouseholds.Tests",
        ...
        Assert.That(topologyIndex, Does.Contain("V845-V852 Household Mobility Runtime Pressure Contribution Floor Extraction"));
        Assert.That(socialStrata, Does.Contain("Current household mobility runtime pressure contribution floor extraction: v845-v852"));
        Assert.That(designAudit, Does.Contain("v845-v852 household mobility runtime pressure contribution floor extraction audit"));
        Assert.That(moduleBoundaries, Does.Contain("Household mobility runtime pressure contribution floor extraction v845-v852 boundary note"));
        Assert.That(integrationRules, Does.Contain("Household mobility runtime pressure contribution floor extraction v845-v852 integration note"));
        Assert.That(simulation, Does.Contain("Current household mobility runtime pressure contribution floor extraction v845-v852 note"));
        Assert.That(uiPresentation, Does.Contain("v845-v852 household mobility runtime pressure contribution floor extraction"));
        Assert.That(acceptance, Does.Contain("Household mobility runtime pressure contribution floor extraction v845-v852 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V845-V852 Household Mobility Runtime Pressure Contribution Floor Extraction"));
        Assert.That(skillMatrix, Does.Contain("Household Mobility Runtime Pressure Contribution Floor Extraction Through V852"));
        Assert.That(schemaRules, Does.Contain("household mobility runtime pressure contribution floor extraction v845-v852 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current household mobility runtime pressure contribution floor extraction v845-v852 note"));

        foreach (string requiredPlanText in new[]
                 {
                     "Target schema/migration impact: none",
                     "owner-consumed hardcoded contribution-floor extraction",
                     "default pressure contribution floor remains `0`",
                     "Runtime behavior under default rules-data: unchanged",
        ...
        Assert.That(rulesData, Does.Contain("MonthlyRuntimePressureContributionFloor"));
        Assert.That(rulesData, Does.Contain("DefaultMonthlyRuntimePressureContributionFloor = 0"));
        Assert.That(rulesData, Does.Contain("MaxMonthlyRuntimePressureContributionFloor"));
        Assert.That(rulesData, Does.Contain("GetMonthlyRuntimePressureContributionFloorOrDefault"));
        Assert.That(rulesData, Does.Contain("monthly_runtime_pressure_contribution_floor"));

        Assert.That(populationModule, Does.Contain("GetMonthlyRuntimePressureContributionFloorOrDefault"));
        Assert.That(populationModule, Does.Contain("pressureContributionFloor"));
        Assert.That(populationModule, Does.Contain("Math.Max(pressureContributionFloor, laborCapacityPressureFloor - household.LaborCapacity)"));
        Assert.That(populationModule, Does.Contain("Math.Max(pressureContributionFloor, grainStorePressureFloor - household.GrainStore)"));
        Assert.That(populationModule, Does.Contain("Math.Max(pressureContributionFloor, landHoldingPressureFloor - household.LandHolding)"));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        ...
        Assert.That(runtimeEnd, Is.GreaterThan(runtimeStart));
        string runtimeRule = populationModule[runtimeStart..runtimeEnd];
        Assert.That(runtimeRule, Does.Contain("GetMonthlyRuntimePressureContributionFloorOrDefault"));
        Assert.That(runtimeRule, Does.Contain("pressureContributionFloor"));
        Assert.That(
            Regex.Matches(populationModule, @"\bApplyMonthlyHouseholdMobilityRuntimeRule\s*\(").Count,
            Is.EqualTo(2),
            "The pressure contribution floor extraction must not add or duplicate the monthly household mobility runtime rule path.");
        ...
        Assert.That(scoreEnd, Is.GreaterThan(scoreStart));
        string scoreMethod = populationModule[scoreStart..scoreEnd];
        Assert.That(scoreMethod, Does.Contain("pressureContributionFloor"));
        Assert.That(scoreMethod, Does.Not.Contain("Math.Max(0,"));

        Assert.That(
            householdModuleTests,
            Does.Contain("RunMonth_FirstMobilityRuntimeRuleDefaultPressureContributionFloorPreservesPreviousScoreContributions"));
        Assert.That(
            householdModuleTests,
            Does.Contain("PopulationHouseholdMobilityRulesData_InvalidMonthlyRuntimePressureContributionFloorFallsBackToDefault"));
        Assert.That(populationState, Does.Not.Contain("MonthlyRuntimePressureContributionFloor"));
        Assert.That(populationState, Does.Not.Contain("HouseholdMobility"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Cooldown"));
        Assert.That(populationState, Does.Not.Contain("PressureContributionLedger"));
        Assert.That(populationState, Does.Not.Contain("OrderingLedger"));
        ...
        foreach (string authorityToken in new[]
                 {
                     ...
                     "HouseholdMobilityPressureContributionInterpreter",
                     "HouseholdMobilityEventRouter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string personRegistryToken in new[]
                 {
                     ...
                     "CommonerStatus",
                     "SocialClass",
                     "PressureContributionFloorExtraction",
                 })
        {
            Assert.That(personRegistrySource, Does.Not.Contain(personRegistryToken), personRegistryToken);
        }
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "HouseholdMobilityPressureContributionLedger",
                     "HouseholdMobilityOrderingLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
    ...
    [Test]
    public void Household_mobility_runtime_extraction_closeout_v853_v860_must_classify_remaining_guards_without_schema_or_authority_drift()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        Assert.That(topologyIndex, Does.Contain("V853-V860 Household Mobility Runtime Extraction Closeout"));
        Assert.That(socialStrata, Does.Contain("Current household mobility runtime extraction closeout: v853-v860"));
        Assert.That(designAudit, Does.Contain("v853-v860 household mobility runtime extraction closeout audit"));
        Assert.That(moduleBoundaries, Does.Contain("Household mobility runtime extraction closeout v853-v860 boundary note"));
        Assert.That(integrationRules, Does.Contain("Household mobility runtime extraction closeout v853-v860 integration note"));
        Assert.That(simulation, Does.Contain("Current household mobility runtime extraction closeout v853-v860 note"));
        Assert.That(uiPresentation, Does.Contain("v853-v860 household mobility runtime extraction closeout"));
        Assert.That(acceptance, Does.Contain("Household mobility runtime extraction closeout v853-v860 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V853-V860 Household Mobility Runtime Extraction Closeout"));
        Assert.That(skillMatrix, Does.Contain("Household Mobility Runtime Extraction Closeout Through V860"));
        Assert.That(schemaRules, Does.Contain("household mobility runtime extraction closeout v853-v860 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current household mobility runtime extraction closeout v853-v860 note"));

        foreach (string requiredPlanText in new[]
                 {
                     "hardcoded extraction closeout",
                     "Runtime behavior change: none",
                     "Target schema/migration impact: none",
                     "remaining inline controls",
                     "not authored rules-data knobs",
                     "No second household mobility runtime rule",
                     "No rules-data loader",
                     "No rules-data file",
        ...
        foreach (string extractedRulesDataToken in new[]
                 {
                     "MonthlyRuntimeActivePoolOutflowThreshold",
                     "MonthlyRuntimeCandidateMigrationRiskFloor",
                     "MonthlyRuntimeCandidateMigrationRiskCeiling",
        ...
        {
            Assert.That(rulesData, Does.Contain(extractedRulesDataToken), extractedRulesDataToken);
        }

        foreach (string runtimeGetter in new[]
                 {
                     ...
                     "GetMonthlyRuntimeLandHoldingPressureFloorOrDefault",
                     "GetMonthlyRuntimeLandHoldingPressureDivisorOrDefault",
                     "GetMonthlyRuntimeSettlementCapOrDefault",
                     "GetMonthlyRuntimeHouseholdCapOrDefault",
                     "GetMonthlyRuntimePoolTieBreakPriorityOrDefault",
                     "GetMonthlyRuntimeHouseholdTieBreakPriorityOrDefault",
                     "GetMonthlyRuntimeRiskDeltaOrDefault",
                     "GetMonthlyRuntimeMigrationRiskClampFloorOrDefault",
                     "GetMonthlyRuntimeMigrationRiskClampCeilingOrDefault",
                     "GetMonthlyRuntimeMigrationStatusThresholdOrDefault",
                     "GetMonthlyRuntimeMigrationStartedEventThresholdOrDefault",
        ...
        {
            Assert.That(populationModule, Does.Contain(runtimeGetter), runtimeGetter);
        }
        ...
        Assert.That(runtimeEnd, Is.GreaterThan(runtimeStart));
        string runtimeRule = populationModule[runtimeStart..runtimeEnd];
        foreach (string remainingControl in new[]
                 {
                     "settlementCap <= 0",
                     "householdCap <= 0",
                     "riskDelta <= 0",
                     ...
                     "bool changed = false",
                     "changed = true",
                     "oldMigrationRisk < migrationStartedEventThreshold",
                     "household.MigrationRisk >= migrationStartedEventThreshold",
                 })
        {
            Assert.That(runtimeRule, Does.Contain(remainingControl), remainingControl);
        }

        Assert.That(
            Regex.Matches(populationModule, @"\bApplyMonthlyHouseholdMobilityRuntimeRule\s*\(").Count,
            Is.EqualTo(2),
            "The extraction closeout must not add or duplicate the monthly household mobility runtime rule path.");
        ...
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Cooldown"));
        Assert.That(populationState, Does.Not.Contain("SelectorWatermark"));
        Assert.That(populationState, Does.Not.Contain("TargetCardinality"));
        ...
        foreach (string authorityToken in new[]
                 {
                     ...
                     "MonthlyRuntimeMigrationStatusThreshold",
                     "MonthlyRuntimeMigrationStartedEventThreshold",
                     "HouseholdMobilityGuardInterpreter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string personRegistryToken in new[]
                 {
                     ...
                     "CommonerStatus",
                     "SocialClass",
                     "HouseholdMobilityExtractionCloseout",
                 })
        {
            Assert.That(personRegistrySource, Does.Not.Contain(personRegistryToken), personRegistryToken);
        }
    ...
    [Test]
    public void Population_households_pool_rebuild_file_split_v877_v884_must_preserve_owner_behavior_and_schema_neutrality()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_population-households-pool-rebuild-file-split-v877-v884.md"));
        string mainModuleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.cs"));
        string poolRebuildFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.PoolRebuild.cs"));
        string populationModule = ReadPopulationAndHouseholdsModuleSource();
        string populationState = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
        ...
        Assert.That(topologyIndex, Does.Contain("V877-V884 PopulationAndHouseholds Pool Rebuild File Split"));
        Assert.That(socialStrata, Does.Contain("Current population households pool rebuild file split: v877-v884"));
        Assert.That(designAudit, Does.Contain("v877-v884 population households pool rebuild file split audit"));
        Assert.That(moduleBoundaries, Does.Contain("PopulationAndHouseholds pool rebuild file split v877-v884 boundary note"));
        Assert.That(integrationRules, Does.Contain("PopulationAndHouseholds pool rebuild file split v877-v884 integration note"));
        Assert.That(simulation, Does.Contain("Current population households pool rebuild file split v877-v884 note"));
        Assert.That(uiPresentation, Does.Contain("v877-v884 population households pool rebuild file split"));
        Assert.That(acceptance, Does.Contain("PopulationAndHouseholds pool rebuild file split v877-v884 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V877-V884 PopulationAndHouseholds Pool Rebuild File Split"));
        Assert.That(skillMatrix, Does.Contain("PopulationAndHouseholds Pool Rebuild File Split Through V884"));
        Assert.That(schemaRules, Does.Contain("population households pool rebuild file split v877-v884 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current population households pool rebuild file split v877-v884 note"));
        ...
        Assert.That(mainModuleFile, Does.Contain("RebuildSettlementSummaries(scope.State, personQueries)"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static void RebuildSettlementSummaries"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static void RebuildLaborPools"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static int ComputeHouseholdLaborDemand"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static void RebuildMarriagePools"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static bool IsMarriagePoolEligible"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static void RebuildMigrationPools"));

        Assert.That(poolRebuildFile, Does.Contain("public sealed partial class PopulationAndHouseholdsModule"));
        Assert.That(poolRebuildFile, Does.Contain("private static void RebuildSettlementSummaries"));
        Assert.That(poolRebuildFile, Does.Contain("private static void RebuildLaborPools"));
        Assert.That(poolRebuildFile, Does.Contain("private static int ComputeHouseholdLaborDemand"));
        Assert.That(poolRebuildFile, Does.Contain("private static void RebuildMarriagePools"));
        Assert.That(poolRebuildFile, Does.Contain("private static bool IsMarriagePoolEligible"));
        Assert.That(poolRebuildFile, Does.Contain("private static void RebuildMigrationPools"));
        Assert.That(poolRebuildFile, Does.Contain(".GroupBy(static household => household.SettlementId)"));
        Assert.That(poolRebuildFile, Does.Contain(".OrderBy(static group => group.Key.Value)"));
        Assert.That(poolRebuildFile, Does.Contain("group.OrderBy(static household => household.Id.Value)"));
        Assert.That(poolRebuildFile, Does.Contain(".OrderBy(static membership => membership.PersonId.Value)"));
        Assert.That(poolRebuildFile, Does.Contain("households.Sum(ComputeHouseholdLaborDemand)"));
        Assert.That(poolRebuildFile, Does.Contain("Math.Clamp(50 + ((laborDemand - availableLabor) / households.Length) + (averageMigration / 4), 20, 120)"));
        Assert.That(poolRebuildFile, Does.Contain("Math.Clamp((mismatch * 12) + averageDistress / 3 + averageMigration / 4, 0, 100)"));
        Assert.That(poolRebuildFile, Does.Contain("Math.Clamp(averageMigration + migratingHouseholds * 8 + migratingMembers * 2, 0, 100)"));

        Assert.That(Regex.Matches(poolRebuildFile, @"private static void RebuildSettlementSummaries\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(poolRebuildFile, @"private static void RebuildLaborPools\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(poolRebuildFile, @"private static int ComputeHouseholdLaborDemand\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(poolRebuildFile, @"private static void RebuildMarriagePools\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(poolRebuildFile, @"private static bool IsMarriagePoolEligible\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(poolRebuildFile, @"private static void RebuildMigrationPools\s*\(").Count, Is.EqualTo(1));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        Assert.That(populationState, Does.Not.Contain("PoolRebuild"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Ledger"));

        foreach (string authorityToken in new[]
                 {
                     "PopulationAndHouseholdsModule.PoolRebuild",
                     "PopulationAndHouseholdsPoolRebuildFileSplit",
                     "PoolRebuildFormulaAuthority",
                     "HouseholdMobilityPoolInterpreter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "PopulationAndHouseholdsPoolRebuildFileSplitState",
                     "PoolRebuildLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
    ...
    [Test]
    public void Population_households_query_surface_file_split_v885_v892_must_preserve_owner_behavior_and_schema_neutrality()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_population-households-query-surface-file-split-v885-v892.md"));
        string mainModuleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.cs"));
        string queriesFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.Queries.cs"));
        string populationModule = ReadPopulationAndHouseholdsModuleSource();
        string populationState = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
        ...
        Assert.That(topologyIndex, Does.Contain("V885-V892 PopulationAndHouseholds Query Surface File Split"));
        Assert.That(socialStrata, Does.Contain("Current population households query surface file split: v885-v892"));
        Assert.That(designAudit, Does.Contain("v885-v892 population households query surface file split audit"));
        Assert.That(moduleBoundaries, Does.Contain("PopulationAndHouseholds query surface file split v885-v892 boundary note"));
        Assert.That(integrationRules, Does.Contain("PopulationAndHouseholds query surface file split v885-v892 integration note"));
        Assert.That(simulation, Does.Contain("Current population households query surface file split v885-v892 note"));
        Assert.That(uiPresentation, Does.Contain("v885-v892 population households query surface file split"));
        Assert.That(acceptance, Does.Contain("PopulationAndHouseholds query surface file split v885-v892 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V885-V892 PopulationAndHouseholds Query Surface File Split"));
        Assert.That(skillMatrix, Does.Contain("PopulationAndHouseholds Query Surface File Split Through V892"));
        Assert.That(schemaRules, Does.Contain("population households query surface file split v885-v892 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current population households query surface file split v885-v892 note"));
        ...
        Assert.That(mainModuleFile, Does.Contain("queries.Register<IPopulationAndHouseholdsQueries>(new PopulationQueries(state))"));
        Assert.That(mainModuleFile, Does.Not.Contain("private sealed class PopulationQueries"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static HouseholdPressureSnapshot CloneHousehold"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static PopulationSettlementSnapshot CloneSettlement"));
        Assert.That(mainModuleFile, Does.Not.Contain("private static HouseholdMembershipSnapshot CloneMembership"));

        Assert.That(queriesFile, Does.Contain("public sealed partial class PopulationAndHouseholdsModule"));
        Assert.That(queriesFile, Does.Contain("private sealed class PopulationQueries : IPopulationAndHouseholdsQueries"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<HouseholdPressureSnapshot> GetHouseholds()"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<PopulationSettlementSnapshot> GetSettlements()"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<HouseholdMembershipSnapshot> GetMemberships()"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<LaborPoolEntrySnapshot> GetLaborPools()"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<MarriagePoolEntrySnapshot> GetMarriagePools()"));
        Assert.That(queriesFile, Does.Contain("public IReadOnlyList<MigrationPoolEntrySnapshot> GetMigrationPools()"));
        Assert.That(queriesFile, Does.Contain(".OrderBy(static household => household.Id.Value)"));
        Assert.That(queriesFile, Does.Contain(".OrderBy(static settlement => settlement.SettlementId.Value)"));
        Assert.That(queriesFile, Does.Contain(".OrderBy(static member => member.PersonId.Value)"));
        Assert.That(queriesFile, Does.Contain(".OrderBy(static entry => entry.SettlementId.Value)"));
        Assert.That(queriesFile, Does.Contain("private static HouseholdPressureSnapshot CloneHousehold"));
        Assert.That(queriesFile, Does.Contain("private static PopulationSettlementSnapshot CloneSettlement"));
        Assert.That(queriesFile, Does.Contain("private static HouseholdMembershipSnapshot CloneMembership"));

        Assert.That(Regex.Matches(queriesFile, @"private sealed class PopulationQueries").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(queriesFile, @"private static HouseholdPressureSnapshot CloneHousehold\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(queriesFile, @"private static PopulationSettlementSnapshot CloneSettlement\s*\(").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(queriesFile, @"private static HouseholdMembershipSnapshot CloneMembership\s*\(").Count, Is.EqualTo(1));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        Assert.That(populationState, Does.Not.Contain("QuerySurface"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Ledger"));

        foreach (string authorityToken in new[]
                 {
                     "PopulationAndHouseholdsModule.Queries",
                     "PopulationAndHouseholdsQuerySurfaceFileSplit",
                     "QuerySurfaceFormulaAuthority",
                     "HouseholdMobilityQueryInterpreter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "PopulationAndHouseholdsQuerySurfaceFileSplitState",
                     "QuerySurfaceLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
    ...
    [Test]
    public void Population_households_monthly_pulse_file_split_v917_v924_must_preserve_owner_behavior_and_schema_neutrality()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_population-households-monthly-pulse-file-split-v917-v924.md"));
        string mainModuleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.cs"));
        string monthlyPulseFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.MonthlyPulse.cs"));
        string populationModule = ReadPopulationAndHouseholdsModuleSource();
        string populationState = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
        ...
        Assert.That(topologyIndex, Does.Contain("V917-V924 PopulationAndHouseholds Monthly Pulse File Split"));
        Assert.That(socialStrata, Does.Contain("Current population households monthly pulse file split: v917-v924"));
        Assert.That(designAudit, Does.Contain("v917-v924 population households monthly pulse file split audit"));
        Assert.That(moduleBoundaries, Does.Contain("PopulationAndHouseholds monthly pulse file split v917-v924 boundary note"));
        Assert.That(integrationRules, Does.Contain("PopulationAndHouseholds monthly pulse file split v917-v924 integration note"));
        Assert.That(simulation, Does.Contain("Current population households monthly pulse file split v917-v924 note"));
        Assert.That(uiPresentation, Does.Contain("v917-v924 population households monthly pulse file split"));
        Assert.That(acceptance, Does.Contain("PopulationAndHouseholds monthly pulse file split v917-v924 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V917-V924 PopulationAndHouseholds Monthly Pulse File Split"));
        Assert.That(skillMatrix, Does.Contain("PopulationAndHouseholds Monthly Pulse File Split Through V924"));
        Assert.That(schemaRules, Does.Contain("population households monthly pulse file split v917-v924 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current population households monthly pulse file split v917-v924 note"));
        ...
        foreach (string movedDefinition in new[]
                 {
                     ...
                     "private static int GetClanSupportReserve",
                     "private static int ComputeDebtDelta",
                     "private static int ComputeLaborDelta",
                     "private static int ComputeMigrationDelta",
                     "private static bool ResolveMigrationStatus(PopulationHouseholdState household)",
                     "private static bool ResolveMigrationStatus(PopulationHouseholdState household, int migrationStatusThreshold)",
                 })
        {
            Assert.That(mainModuleFile, Does.Not.Contain(movedDefinition), movedDefinition);
            Assert.That(monthlyPulseFile, Does.Contain(movedDefinition), movedDefinition);
        }

        Assert.That(monthlyPulseFile, Does.Contain("public sealed partial class PopulationAndHouseholdsModule"));
        Assert.That(monthlyPulseFile, Does.Contain("SimulationXun.Shangxun"));
        Assert.That(monthlyPulseFile, Does.Contain("household.DebtPressure >= 50 ? 1 : 0"));
        Assert.That(monthlyPulseFile, Does.Contain("prosperity >= 58 ? 1 : prosperity <= 42 ? -1 : 0"));
        Assert.That(monthlyPulseFile, Does.Contain("distress >= 70 ? 2 : distress >= 50 ? 1 : -1"));
        Assert.That(monthlyPulseFile, Does.Contain("DefaultMonthlyRuntimeMigrationStatusThreshold"));
        Assert.That(monthlyPulseFile, Does.Contain("MigrationRisk >= migrationStatusThreshold"));
        Assert.That(mainModuleFile, Does.Contain("ApplyXunPulseAdjustments(scope.Context.CurrentXun, settlement, clanSupport, household)"));
        Assert.That(mainModuleFile, Does.Contain("ComputeDebtDelta(household.Distress)"));
        Assert.That(mainModuleFile, Does.Contain("ComputeLaborDelta(settlement.Prosperity, household.Distress)"));
        Assert.That(mainModuleFile, Does.Contain("ComputeMigrationDelta(settlement.Security, household.Distress)"));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        Assert.That(populationState, Does.Not.Contain("MonthlyPulse"));
        Assert.That(populationState, Does.Not.Contain("MigrationStatus"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Ledger"));

        foreach (string authorityToken in new[]
                 {
                     "PopulationAndHouseholdsModule.MonthlyPulse",
                     "PopulationAndHouseholdsMonthlyPulseFileSplit",
                     "MonthlyPulseFormulaAuthority",
                     "HouseholdMobilityMonthlyInterpreter",
                     "MonthlyPulseOutcomeCalculator",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "PopulationAndHouseholdsMonthlyPulseFileSplitState",
                     "MonthlyPulseLedger",
                     "MigrationStatusLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
                     "CooldownLedger",
    ...
    [Test]
    public void Population_households_warfare_aftermath_file_split_v925_v932_must_preserve_owner_behavior_and_schema_neutrality()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_population-households-warfare-aftermath-file-split-v925-v932.md"));
        string mainModuleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.cs"));
        string warfareAftermathFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.WarfareAftermath.cs"));
        string populationModule = ReadPopulationAndHouseholdsModuleSource();
        string populationState = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
        ...
        Assert.That(topologyIndex, Does.Contain("V925-V932 PopulationAndHouseholds Warfare Aftermath File Split"));
        Assert.That(socialStrata, Does.Contain("Current population households warfare aftermath file split: v925-v932"));
        Assert.That(designAudit, Does.Contain("v925-v932 population households warfare aftermath file split audit"));
        Assert.That(moduleBoundaries, Does.Contain("PopulationAndHouseholds warfare aftermath file split v925-v932 boundary note"));
        Assert.That(integrationRules, Does.Contain("PopulationAndHouseholds warfare aftermath file split v925-v932 integration note"));
        Assert.That(simulation, Does.Contain("Current population households warfare aftermath file split v925-v932 note"));
        Assert.That(uiPresentation, Does.Contain("v925-v932 population households warfare aftermath file split"));
        Assert.That(acceptance, Does.Contain("PopulationAndHouseholds warfare aftermath file split v925-v932 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V925-V932 PopulationAndHouseholds Warfare Aftermath File Split"));
        Assert.That(skillMatrix, Does.Contain("PopulationAndHouseholds Warfare Aftermath File Split Through V932"));
        Assert.That(schemaRules, Does.Contain("population households warfare aftermath file split v925-v932 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current population households warfare aftermath file split v925-v932 note"));
        ...
        foreach (string movedDefinition in new[]
                 {
                     "private static void ApplyWarfareCampaignAftermathEvents",
                     "private static int ComputeCampaignDistressDelta",
                     "private static int ComputeCampaignDebtDelta",
                     "private static int ComputeCampaignMigrationDelta",
                     "private static int ComputeCampaignLaborDrop",
                 })
        {
            Assert.That(mainModuleFile, Does.Not.Contain(movedDefinition), movedDefinition);
            Assert.That(warfareAftermathFile, Does.Contain(movedDefinition), movedDefinition);
        }

        Assert.That(warfareAftermathFile, Does.Contain("public sealed partial class PopulationAndHouseholdsModule"));
        Assert.That(warfareAftermathFile, Does.Contain("WarfareCampaignEventBundler.Build(scope.Events)"));
        Assert.That(warfareAftermathFile, Does.Contain(".OrderBy(static household => household.Id.Value)"));
        Assert.That(warfareAftermathFile, Does.Contain("ComputeCampaignDistressDelta(bundle, campaign)"));
        Assert.That(warfareAftermathFile, Does.Contain("PopulationEventNames.MigrationStarted"));
        Assert.That(warfareAftermathFile, Does.Contain("PopulationEventNames.LivelihoodCollapsed"));
        Assert.That(warfareAftermathFile, Does.Contain("RebuildSettlementSummaries(scope.State, scope.TryGetQuery<IPersonRegistryQueries>())"));
        Assert.That(mainModuleFile, Does.Contain("DispatchTradeShockEvents(scope);"));
        Assert.That(mainModuleFile, Does.Contain("DispatchWorldPulseEvents(scope);"));
        Assert.That(mainModuleFile, Does.Contain("DispatchFamilyBranchEvents(scope);"));
        Assert.That(mainModuleFile, Does.Contain("DispatchOfficeSupplyEvents(scope);"));
        Assert.That(mainModuleFile, Does.Contain("ApplyWarfareCampaignAftermathEvents(scope);"));
        Assert.That(
            mainModuleFile.IndexOf("DispatchOfficeSupplyEvents(scope);", StringComparison.Ordinal),
            Is.LessThan(mainModuleFile.IndexOf("ApplyWarfareCampaignAftermathEvents(scope);", StringComparison.Ordinal)));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        Assert.That(populationState, Does.Not.Contain("WarfareAftermath"));
        Assert.That(populationState, Does.Not.Contain("CampaignAftermath"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Ledger"));

        foreach (string authorityToken in new[]
                 {
                     "PopulationAndHouseholdsModule.WarfareAftermath",
                     "PopulationAndHouseholdsWarfareAftermathFileSplit",
                     "WarfareAftermathFormulaAuthority",
                     "HouseholdMobilityWarfareInterpreter",
                     "CampaignDeltaOutcomeCalculator",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "PopulationAndHouseholdsWarfareAftermathFileSplitState",
                     "WarfareAftermathLedger",
                     "CampaignAftermathLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
                     "OwnerLaneLedger",
                     "CooldownLedger",
    ...
    [Test]
    public void Population_households_health_lifecycle_file_split_v933_v940_must_preserve_owner_behavior_and_schema_neutrality()
    {
        string topologyIndex = File.ReadAllText(Path.Combine(RepoRoot, "docs", "RENZONG_THIN_CHAIN_TOPOLOGY_INDEX.md"));
        string socialStrata = File.ReadAllText(Path.Combine(RepoRoot, "docs", "SOCIAL_STRATA_AND_PATHWAYS.md"));
        string designAudit = File.ReadAllText(Path.Combine(RepoRoot, "docs", "DESIGN_CODE_ALIGNMENT_AUDIT.md"));
        ...
        string execPlan = File.ReadAllText(Path.Combine(
            ...
            "exec-plans",
            "active",
            "2026-05-03_population-households-health-lifecycle-file-split-v933-v940.md"));
        string mainModuleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.cs"));
        string healthLifecycleFile = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
            "PopulationAndHouseholdsModule.HealthLifecycle.cs"));
        string populationModule = ReadPopulationAndHouseholdsModuleSource();
        string populationState = File.ReadAllText(Path.Combine(
            SrcDir,
            "Zongzu.Modules.PopulationAndHouseholds",
        ...
        Assert.That(topologyIndex, Does.Contain("V933-V940 PopulationAndHouseholds Health Lifecycle File Split"));
        Assert.That(socialStrata, Does.Contain("Current population households health lifecycle file split: v933-v940"));
        Assert.That(designAudit, Does.Contain("v933-v940 population households health lifecycle file split audit"));
        Assert.That(moduleBoundaries, Does.Contain("PopulationAndHouseholds health lifecycle file split v933-v940 boundary note"));
        Assert.That(integrationRules, Does.Contain("PopulationAndHouseholds health lifecycle file split v933-v940 integration note"));
        Assert.That(simulation, Does.Contain("Current population households health lifecycle file split v933-v940 note"));
        Assert.That(uiPresentation, Does.Contain("v933-v940 population households health lifecycle file split"));
        Assert.That(acceptance, Does.Contain("PopulationAndHouseholds health lifecycle file split v933-v940 acceptance"));
        Assert.That(fidelityModel, Does.Contain("V933-V940 PopulationAndHouseholds Health Lifecycle File Split"));
        Assert.That(skillMatrix, Does.Contain("PopulationAndHouseholds Health Lifecycle File Split Through V940"));
        Assert.That(schemaRules, Does.Contain("population households health lifecycle file split v933-v940 adds no persisted fields"));
        Assert.That(dataSchema, Does.Contain("Current population households health lifecycle file split v933-v940 note"));
        ...
        foreach (string requiredPlanText in new[]
                 {
                     ...
                     "Target schema/migration impact: none",
                     "PopulationAndHouseholdsModule.HealthLifecycle.cs",
                     "No health lifecycle behavior change",
                     "No illness threshold change",
                     "No mortality threshold change",
                     "No illness-month cap change",
                     "No dependent-count behavior change",
                     "No `PersonRegistry` schema or domain expansion",
                     "No event behavior change",
                     "No emitted metadata change",
                     "No receipt/projection text rewrite",
                     "No rules-data parameter change",
                     ...
                     "No persisted state",
                     "No schema bump",
                     "Application/UI/Unity do not calculate health lifecycle outcomes, illness-death outcomes, household mobility outcomes, or household pressure results",
                     "No authored rules-data externalization in this split",
                 })
        {
            Assert.That(execPlan, Does.Contain(requiredPlanText), requiredPlanText);
        ...
        Assert.That(mainModuleFile, Does.Not.Contain("private static void AdvanceIllnessAndAdjudicateDeaths"));
        Assert.That(mainModuleFile, Does.Contain("AdvanceIllnessAndAdjudicateDeaths(scope);"));
        int promoteIndex = mainModuleFile.IndexOf("PromoteHotHouseholdMembers(", StringComparison.Ordinal);
        int healthIndex = mainModuleFile.IndexOf("AdvanceIllnessAndAdjudicateDeaths(scope);", promoteIndex, StringComparison.Ordinal);
        int rebuildAfterHealthIndex = mainModuleFile.IndexOf("RebuildSettlementSummaries(scope.State, personQueries);", healthIndex, StringComparison.Ordinal);
        Assert.That(promoteIndex, Is.GreaterThanOrEqualTo(0));
        Assert.That(healthIndex, Is.GreaterThan(promoteIndex));
        Assert.That(rebuildAfterHealthIndex, Is.GreaterThan(healthIndex));

        Assert.That(healthLifecycleFile, Does.Contain("public sealed partial class PopulationAndHouseholdsModule"));
        Assert.That(healthLifecycleFile, Does.Contain("private static void AdvanceIllnessAndAdjudicateDeaths"));
        Assert.That(healthLifecycleFile, Does.Contain("IPersonRegistryCommands registryCommands = scope.GetRequiredQuery<IPersonRegistryCommands>()"));
        Assert.That(healthLifecycleFile, Does.Contain(".OrderBy(static member => member.PersonId.Value)"));
        Assert.That(healthLifecycleFile, Does.Contain("membership.IllnessMonths = Math.Min(membership.IllnessMonths + 1, 24)"));
        Assert.That(healthLifecycleFile, Does.Contain("membership.IllnessMonths = Math.Max(membership.IllnessMonths - 1, 0)"));
        Assert.That(healthLifecycleFile, Does.Contain("registryCommands.MarkDeceased(scope.Context, membership.PersonId)"));
        Assert.That(healthLifecycleFile, Does.Contain("PopulationEventNames.DeathByIllness"));
        Assert.That(healthLifecycleFile, Does.Contain("household.DependentCount = Math.Max(0, household.DependentCount - 1)"));
        Assert.That(populationModule, Does.Contain("ModuleSchemaVersion => 3"));
        Assert.That(populationState, Does.Not.Contain("HealthLifecycle"));
        Assert.That(populationState, Does.Not.Contain("Mortality"));
        Assert.That(populationState, Does.Not.Contain("RouteHistory"));
        Assert.That(populationState, Does.Not.Contain("Ledger"));

        foreach (string authorityToken in new[]
                 {
                     "PopulationAndHouseholdsModule.HealthLifecycle",
                     "PopulationAndHouseholdsHealthLifecycleFileSplit",
                     "HealthLifecycleOutcomeCalculator",
                     "IllnessDeathOutcomeCalculator",
                     "HouseholdMobilityHealthInterpreter",
                 })
        {
            Assert.That(applicationSource, Does.Not.Contain(authorityToken), authorityToken);
            Assert.That(presentationSource, Does.Not.Contain(authorityToken), authorityToken);
        ...
        foreach (string forbidden in new[]
                 {
                     ...
                     "CommonerStatusEngine",
                     "SocialClassEngine",
                     "PopulationAndHouseholdsHealthLifecycleFileSplitState",
                     "HealthLifecycleLedger",
                     "IllnessLedger",
                     "MortalityLedger",
                     "MobilitySelectorWatermark",
                     "TargetCardinalityState",
        ...
        return string.Join(
            ...
            EnumerateSourceFiles(Path.Combine(SrcDir, "Zongzu.Modules.PopulationAndHouseholds"))
                .OrderBy(static file => Path.GetFileName(file) switch
                {
                    ...
                    "PopulationAndHouseholdsModule.MonthlyPulse.cs" => 6,
                    "PopulationAndHouseholdsModule.LivelihoodDrift.cs" => 7,
                    "PopulationAndHouseholdsModule.WarfareAftermath.cs" => 8,
                    "PopulationAndHouseholdsModule.HealthLifecycle.cs" => 9,
                    "PopulationAndHouseholdsModule.cs" => 10,
                    _ => 11,
                })
